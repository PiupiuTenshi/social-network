# PH03-FE-SET-01-P0-01 — Triển khai SET-01 P0: Quyền riêng tư

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: FE | Ưu tiên: P0 | Task cha: PH03-FE-SET-01-P0.
- Mục tiêu duy nhất: Triển khai SET-01 P0: Quyền riêng tư.
- Miền: cross-cutting | Chức năng: ACC-07.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-FE-SET-01-P0-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /thiet-lap/quyen-rieng-tu. Vai trò: Người dùng. Chỉ bật ACC-07 tại P0. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-FE-SET-01-P0-01
...
1. Mục tiêu screen: Thiết lập mức hiển thị hồ sơ, bài viết, trạng thái hiện diện và lời mời nhắn tin.
...
- **AC01**: Màn hình SET-01 tại /thiet-lap/quyen-rieng-tu thực hiện đúng quyền Người dùng và capability ACC-07.
- **AC02**: Trạng thái riêng được kiểm chứng: - Đã lưu; - Có thay đổi chưa lưu; - Dữ liệu không hợp lệ; - Mất kết nối; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH03-FE-SET-01-P0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
