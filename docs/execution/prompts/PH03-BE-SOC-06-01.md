# PH03-BE-SOC-06-01 — Triển khai SOC-06: Xem bài viết và danh sách bài viết

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-SOC-06.
- Mục tiêu duy nhất: Triển khai SOC-06: Xem bài viết và danh sách bài viết.
- Miền: social | Chức năng: SOC-06.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-SOC-06-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội. Hợp đồng tham chiếu: GET /posts; /users/{id}/posts. Dữ liệu: post. Sự kiện: Không.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-SOC-06-01
...
1. Theo hợp đồng đã khóa: Áp dụng bộ lọc mức hiển thị và trạng thái chặn -> truy vấn bằng con trỏ ổn định.
2. Thực thi: Visibility + block; cursor ổn định.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của SOC-06, when thực hiện hợp lệ, then Áp dụng bộ lọc mức hiển thị và trạng thái chặn -> truy vấn bằng con trỏ ổn định.
- **AC02**: Kiểm soát được chứng minh: Visibility + block; cursor ổn định.
- **AC03**: Ca tối thiểu chạy thật: Ẩn nội dung; cursor không trùng/bỏ.
...
Ghi `TASK_ID=PH03-BE-SOC-06-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
