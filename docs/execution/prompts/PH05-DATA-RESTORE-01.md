# PH05-DATA-RESTORE-01 — Khôi phục Media và dữ liệu P1

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: DATA | Ưu tiên: P1 | Task cha: PH05-DATA-RESTORE.
- Mục tiêu duy nhất: Khôi phục Media và dữ liệu P1.
...
- Nhánh đề xuất: `feat/PH05-DATA-RESTORE-01`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-DATA-RESTORE-01
...
1. Khôi phục metadata trước object; đối soát object key/size/hash và asset thiếu.
2. Diễn tập retention_hold, audit, retry và deletion ledger sau restore.

## Tiêu chí chấp nhận

- **AC01**: Media RPO <=24h/RTO <=8h; dữ liệu P0 vẫn đạt <=4h và smoke đầy đủ.
- **AC02**: Object không public; audit/retention và dữ liệu xóa không bị phục hồi sai.
...
Ghi `TASK_ID=PH05-DATA-RESTORE-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
