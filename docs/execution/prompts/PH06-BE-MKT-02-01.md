# PH06-BE-MKT-02-01 — Triển khai MKT-02: Duyệt và tìm kiếm tin đăng

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-MKT-02.
- Mục tiêu duy nhất: Triển khai MKT-02: Duyệt và tìm kiếm tin đăng.
- Miền: commerce | Chức năng: MKT-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-MKT-02-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Thương mại. Hợp đồng tham chiếu: GET /listings. Dữ liệu: listing. Sự kiện: Không.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-MKT-02-01
...
1. Theo hợp đồng đã khóa: Truy vấn bằng cursor và bộ lọc; dùng PostgreSQL FTS làm cơ sở hoặc OpenSearch khi được bật.
2. Thực thi: Active filter; cursor.
3. Khảo sát src/Services/Commerce, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MKT-02, when thực hiện hợp lệ, then Truy vấn bằng cursor và bộ lọc; dùng PostgreSQL FTS làm cơ sở hoặc OpenSearch khi được bật.
- **AC02**: Kiểm soát được chứng minh: Active filter; cursor.
- **AC03**: Ca tối thiểu chạy thật: Search down -> DB fallback.
...
Ghi `TASK_ID=PH06-BE-MKT-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
