# PH06-BE-AI-04-01 — Triển khai AI-04: Tóm tắt tin nhắn hoặc kênh

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-AI-04.
- Mục tiêu duy nhất: Triển khai AI-04: Tóm tắt tin nhắn hoặc kênh.
- Miền: ai | Chức năng: AI-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-AI-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: AI và tìm kiếm + trò chuyện. Hợp đồng tham chiếu: POST /ai/summaries. Dữ liệu: summary tùy chọn. Sự kiện: AiSummaryReady.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-AI-04-01
...
1. Theo hợp đồng đã khóa: Trò chuyện cung cấp nội dung đã giới hạn hoặc hợp đồng xuất dữ liệu -> AI tóm tắt -> trả kết quả.
2. Thực thi: Trò chuyện cấp nội dung; không query chat_db.
3. Khảo sát src/Services/AiSearch, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của AI-04, when thực hiện hợp lệ, then Trò chuyện cung cấp nội dung đã giới hạn hoặc hợp đồng xuất dữ liệu -> AI tóm tắt -> trả kết quả.
- **AC02**: Kiểm soát được chứng minh: Trò chuyện cấp nội dung; không query chat_db.
- **AC03**: Ca tối thiểu chạy thật: Large chunk; deleted messages.
...
Ghi `TASK_ID=PH06-BE-AI-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
