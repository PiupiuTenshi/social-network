# PH06-BE-MKT-07-02 — Nghiệm thu MKT-07 và bàn giao

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-MKT-07.
- Mục tiêu duy nhất: Nghiệm thu MKT-07 và bàn giao.
- Miền: commerce | Chức năng: MKT-07.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-MKT-07-02`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-MKT-07-02
...
1. Rà soát MKT-07 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Shared media không bị xóa sớm.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí MKT-07 có report: Shared media không bị xóa sớm; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH06-BE-MKT-07-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
