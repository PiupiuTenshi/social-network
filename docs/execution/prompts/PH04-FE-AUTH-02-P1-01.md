# PH04-FE-AUTH-02-P1-01 — Triển khai AUTH-02 P1: Đăng nhập

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: FE | Ưu tiên: P1 | Task cha: PH04-FE-AUTH-02-P1.
- Mục tiêu duy nhất: Triển khai AUTH-02 P1: Đăng nhập.
- Miền: cross-cutting | Chức năng: ACC-10, ACC-08.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-FE-AUTH-02-P1-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /dang-nhap. Vai trò: Khách. Chỉ bật ACC-10, ACC-08 tại P1. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-FE-AUTH-02-P1-01
...
1. Mục tiêu screen: Xác thực bằng email và mật khẩu, đồng thời cung cấp lối vào Google OAuth và khôi phục mật khẩu.
...
- **AC01**: Màn hình AUTH-02 tại /dang-nhap thực hiện đúng quyền Khách và capability ACC-10, ACC-08.
- **AC02**: Trạng thái riêng được kiểm chứng: - Rỗng; - Đang gửi; - Sai thông tin; - Tạm khóa do quá nhiều lần thử; - Dịch vụ tạm thời không khả dụng; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH04-FE-AUTH-02-P1-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
