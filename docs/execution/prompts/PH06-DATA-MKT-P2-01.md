# PH06-DATA-MKT-P2-01 — Thiết kế lược đồ MKT P2

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: DATA | Ưu tiên: P2 | Task cha: PH06-DATA-MKT-P2.
- Mục tiêu duy nhất: Thiết kế lược đồ MKT P2.
- Miền: commerce | Chức năng: nền tảng/quy trình.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-DATA-MKT-P2-01`; chưa được tạo chỉ vì có trong prompt.
- listing/media_ref/inventory/order/item/payment/history; số tiền snapshot, reserved constraint, idempotency

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-DATA-MKT-P2-01
...
1. Cụ thể hóa listing/media_ref/inventory/order/item/payment/history; số tiền snapshot, reserved constraint, idempotency.
2. Đối chiếu ERD nguồn và hợp đồng đã khóa; chọn index từ query, mapping UTC/version, data classification.

## Tiêu chí chấp nhận

- **AC01**: ERD và mapping cụ thể cho listing/media_ref/inventory/order/item/payment/history; số tiền snapshot, reserved constraint, idempotency; không lấn ownership.
- **AC02**: Mỗi constraint/index có bất biến hoặc truy vấn chứng minh; kế hoạch migration/retention được review.
...
Ghi `TASK_ID=PH06-DATA-MKT-P2-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
