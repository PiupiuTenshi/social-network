# PH04-BE-ACC-10-01 — Triển khai ACC-10: Đăng nhập bằng Google OAuth

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-ACC-10.
- Mục tiêu duy nhất: Triển khai ACC-10: Đăng nhập bằng Google OAuth.
- Miền: account | Chức năng: ACC-10.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-ACC-10-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: POST /auth/google. Dữ liệu: account_oauth, account, profile. Sự kiện: UserRegistered nếu mới.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-ACC-10-01
...
1. Theo hợp đồng đã khóa: Xác minh id_token với Google theo issuer, audience và chữ ký -> tìm account_oauth theo (provider, provider_user_id) -> nếu chưa có thì tạo account, account_oauth và profile mặc định -> cấp access token/refresh token như ACC-02.
2. Thực thi: issuer/audience/signature; không tự gộp tài khoản.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-10, when thực hiện hợp lệ, then Xác minh id_token với Google theo issuer, audience và chữ ký -> tìm account_oauth theo (provider, provider_user_id) -> nếu chưa có thì tạo account, account_oauth và profile mặc định -> cấp access token/refresh token như ACC-02.
- **AC02**: Kiểm soát được chứng minh: issuer/audience/signature; không tự gộp tài khoản.
- **AC03**: Ca tối thiểu chạy thật: Token Google sai; email trùng.
...
Ghi `TASK_ID=PH04-BE-ACC-10-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
