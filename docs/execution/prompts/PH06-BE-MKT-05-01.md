# PH06-BE-MKT-05-01 — Triển khai MKT-05: Thanh toán giả lập

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-MKT-05.
- Mục tiêu duy nhất: Triển khai MKT-05: Thanh toán giả lập.
- Miền: commerce | Chức năng: MKT-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-MKT-05-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Thương mại. Hợp đồng tham chiếu: POST /orders/{id}/pay. Dữ liệu: payment, order. Sự kiện: PaymentSucceeded/Failed.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-MKT-05-01
...
1. Theo hợp đồng đã khóa: Gọi MockPaymentProvider -> lưu kết quả lặp an toàn -> xác nhận hoặc thực hiện bù trừ.
2. Thực thi: Idempotency-Key; amount match.
3. Khảo sát src/Services/Commerce, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MKT-05, when thực hiện hợp lệ, then Gọi MockPaymentProvider -> lưu kết quả lặp an toàn -> xác nhận hoặc thực hiện bù trừ.
- **AC02**: Kiểm soát được chứng minh: Idempotency-Key; amount match.
- **AC03**: Ca tối thiểu chạy thật: Retry trả cùng kết quả.
...
Ghi `TASK_ID=PH06-BE-MKT-05-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
