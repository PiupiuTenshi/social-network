# PH03-BE-CHT-03-01 — Triển khai CHT-03: Sửa và xóa tin nhắn

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-CHT-03.
- Mục tiêu duy nhất: Triển khai CHT-03: Sửa và xóa tin nhắn.
- Miền: chat | Chức năng: CHT-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-CHT-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Trò chuyện. Hợp đồng tham chiếu: PATCH/DELETE /messages/{id}. Dữ liệu: message. Sự kiện: MessageEdited/Deleted.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-CHT-03-01
...
1. Theo hợp đồng đã khóa: Cập nhật lạc quan hoặc xóa mềm -> phát cập nhật SignalR -> phát sự kiện.
2. Thực thi: Sender/moderator; If-Match.
3. Khảo sát src/Services/Chat, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của CHT-03, when thực hiện hợp lệ, then Cập nhật lạc quan hoặc xóa mềm -> phát cập nhật SignalR -> phát sự kiện.
- **AC02**: Kiểm soát được chứng minh: Sender/moderator; If-Match.
- **AC03**: Ca tối thiểu chạy thật: Stale 412; soft delete.
...
Ghi `TASK_ID=PH03-BE-CHT-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
