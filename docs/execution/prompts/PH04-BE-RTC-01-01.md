# PH04-BE-RTC-01-01 — Triển khai RTC-01: Cấp token tham gia LiveKit

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-RTC-01.
- Mục tiêu duy nhất: Triển khai RTC-01: Cấp token tham gia LiveKit.
- Miền: community | Chức năng: RTC-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-RTC-01-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Cộng đồng. Hợp đồng tham chiếu: POST /channels/{id}/rtc-token. Dữ liệu: room metadata tùy chọn. Sự kiện: Lifecycle tùy chọn.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-RTC-01-01
...
1. Theo hợp đồng đã khóa: Kiểm tra thành viên và quyền JoinVoice/Speak -> sinh token sống ngắn, giới hạn phạm vi.
2. Thực thi: Identity từ claims; quyền Join/Speak/Screen.
3. Khảo sát src/Services/Community, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của RTC-01, when thực hiện hợp lệ, then Kiểm tra thành viên và quyền JoinVoice/Speak -> sinh token sống ngắn, giới hạn phạm vi.
- **AC02**: Kiểm soát được chứng minh: Identity từ claims; quyền Join/Speak/Screen.
- **AC03**: Ca tối thiểu chạy thật: Không member; LiveKit down.
...
Ghi `TASK_ID=PH04-BE-RTC-01-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
