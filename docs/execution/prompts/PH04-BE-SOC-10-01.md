# PH04-BE-SOC-10-01 — Triển khai SOC-10: Chia sẻ bài viết ra nền tảng ngoài

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-SOC-10.
- Mục tiêu duy nhất: Triển khai SOC-10: Chia sẻ bài viết ra nền tảng ngoài.
- Miền: social | Chức năng: SOC-10.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-SOC-10-01`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-SOC-10-01
...
1. Theo hợp đồng đã khóa: Người dùng chọn chia sẻ -> máy chủ sinh liên kết kèm Open Graph metadata và ghi post_share -> trả liên kết để máy khách mở thao tác chia sẻ tới nền tảng ngoài.
2. Thực thi: Visibility; link an toàn.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của SOC-10, when thực hiện hợp lệ, then Người dùng chọn chia sẻ -> máy chủ sinh liên kết kèm Open Graph metadata và ghi post_share -> trả liên kết để máy khách mở thao tác chia sẻ tới nền tảng ngoài.
- **AC02**: Kiểm soát được chứng minh: Visibility; link an toàn.
- **AC03**: Ca tối thiểu chạy thật: Nền tảng lạ; copy-link fallback.
...
Ghi `TASK_ID=PH04-BE-SOC-10-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
