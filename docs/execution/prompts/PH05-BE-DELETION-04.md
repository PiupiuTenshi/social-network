# PH05-BE-DELETION-04 — Xóa tài khoản: Vô hiệu membership/role/invite và chuyển owner trước purge

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-DELETION.
- Mục tiêu duy nhất: Xóa tài khoản: Vô hiệu membership/role/invite và chuyển owner trước purge.
...
- Nhánh đề xuất: `feat/PH05-BE-DELETION-04`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-DELETION-04
...
1. Owner xử lý: community; Vô hiệu membership/role/invite và chuyển owner trước purge.
2. Chỉ qua contract/event; ghi ack cùng transaction, lặp an toàn và có metric.

## Tiêu chí chấp nhận

- **AC01**: Vô hiệu membership/role/invite và chuyển owner trước purge có bằng chứng test và artifact được đối soát.
...
Ghi `TASK_ID=PH05-BE-DELETION-04`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
