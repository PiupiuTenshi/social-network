# PH00-BE-CONTRACT-01 — Khóa hợp đồng P0 và xử lý mâu thuẫn nền

## Mục tiêu và phạm vi

- Phase: PH00 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH00-BE-CONTRACT.
- Mục tiêu duy nhất: Khóa hợp đồng P0 và xử lý mâu thuẫn nền.
...
- Nhánh đề xuất: `docs/PH00-BE-CONTRACT-01`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH00-BE-CONTRACT-01
...
1. Giải quyết BL-02/03/04/05/06: lỗi version, API đọc, SignalR, Kafka partition/correlation, privacy propagation.
2. Tạo OpenAPI/schema/AsyncAPI P0 đầy đủ request/response/error/permission; đối chiếu API còn thiếu để dùng thực tế cho UI.
3. Ghi ADR đề xuất và chỉ chốt khi có quyết định; xác định P0 text-only và các khả năng P1/P2 mặc định tắt.

## Tiêu chí chấp nhận

- **AC01**: Mọi API dùng ở P0 có operation, DTO và lỗi khóa; không còn endpoint suy đoán.
- **AC02**: Schema sự kiện có partition key, aggregate version, actor/scope, replay và quyền rõ; các mâu thuẫn có quyết định được chấp thuận.
- **AC03**: Có contract tests/fixtures cho luồng P0; P1/P2 giữ hợp đồng tham chiếu và danh sách quyết định còn mở.
...
Ghi `TASK_ID=PH00-BE-CONTRACT-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
