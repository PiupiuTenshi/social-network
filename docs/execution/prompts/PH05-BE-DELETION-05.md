# PH05-BE-DELETION-05 — Xóa tài khoản: Đối soát tham chiếu, dọn object không dùng, giữ evidence có hold

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-DELETION.
- Mục tiêu duy nhất: Xóa tài khoản: Đối soát tham chiếu, dọn object không dùng, giữ evidence có hold.
...
- Nhánh đề xuất: `feat/PH05-BE-DELETION-05`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-DELETION-05
...
1. Owner xử lý: media; Đối soát tham chiếu, dọn object không dùng, giữ evidence có hold.
2. Chỉ qua contract/event; ghi ack cùng transaction, lặp an toàn và có metric.

## Tiêu chí chấp nhận

- **AC01**: Đối soát tham chiếu, dọn object không dùng, giữ evidence có hold có bằng chứng test và artifact được đối soát.
...
Ghi `TASK_ID=PH05-BE-DELETION-05`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
