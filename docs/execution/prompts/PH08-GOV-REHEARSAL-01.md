# PH08-GOV-REHEARSAL-01 — Diễn tập phát hành từ môi trường sạch

## Mục tiêu và phạm vi

- Phase: PH08 | Nhóm: GOV | Ưu tiên: P0 | Task cha: PH08-GOV-REHEARSAL.
- Mục tiêu duy nhất: Diễn tập phát hành từ môi trường sạch.
...
- Nhánh đề xuất: `feat/PH08-GOV-REHEARSAL-01`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH08-GOV-REHEARSAL-01
...
1. Dùng candidate SHA cố định, dựng hệ thống mới bằng runbook; chạy toàn bộ demo và smoke.
2. Đối chiếu image digest/SBOM/migration/flag, diễn tập deploy/rollback trong môi trường được ủy quyền.

## Tiêu chí chấp nhận

- **AC01**: Clone/config/build/migration/start/demo chạy lặp lại không sửa tay DB.
- **AC02**: Có biên bản thời gian, môi trường, candidate SHA và rollback; lỗi mở còn lại không chặn nghiệm thu.
...
Ghi `TASK_ID=PH08-GOV-REHEARSAL-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
