# PH04-BE-MED-05-01 — Triển khai MED-05: Xóa và dọn dẹp phương tiện

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-MED-05.
- Mục tiêu duy nhất: Triển khai MED-05: Xóa và dọn dẹp phương tiện.
- Miền: media | Chức năng: MED-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-MED-05-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Phương tiện. Hợp đồng tham chiếu: DELETE /media/{id}; cleanup. Dữ liệu: media_asset, object. Sự kiện: MediaDeleted.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-MED-05-01
...
1. Theo hợp đồng đã khóa: Đánh dấu hoặc xóa siêu dữ liệu và đối tượng; nếu cần thì lên lịch dọn dẹp.
2. Thực thi: Reference policy; orphan TTL.
3. Khảo sát src/Services/Media, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MED-05, when thực hiện hợp lệ, then Đánh dấu hoặc xóa siêu dữ liệu và đối tượng; nếu cần thì lên lịch dọn dẹp.
- **AC02**: Kiểm soát được chứng minh: Reference policy; orphan TTL.
- **AC03**: Ca tối thiểu chạy thật: Delete object fail; retry.
...
Ghi `TASK_ID=PH04-BE-MED-05-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
