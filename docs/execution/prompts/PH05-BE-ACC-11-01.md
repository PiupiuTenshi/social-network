# PH05-BE-ACC-11-01 — Triển khai ACC-11: Yêu cầu xóa và khôi phục tài khoản

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-ACC-11.
- Mục tiêu duy nhất: Triển khai ACC-11: Yêu cầu xóa và khôi phục tài khoản.
- Miền: account | Chức năng: ACC-11.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH05-BE-ACC-11-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: DELETE /users/me; cancel. Dữ liệu: account_deletion_request. Sự kiện: UserDeletionRequested/Cancelled/Completed. Hoàn thành API Account chưa thay cho subtask xóa của từng downstream trong PH05-BE-DELETION.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-ACC-11-01
...
1. Theo hợp đồng đã khóa: Tạo account_deletion_request -> đặt account.status=pending_deletion -> thu hồi mọi refresh token -> phát UserDeletionRequested -> các dịch vụ ẩn dữ liệu và xác nhận -> sau 30 ngày tiến trình purge xóa/ẩn danh dữ liệu -> phát UserDeletionCompleted.
2. Thực thi: Xác thực lại; grace period; audit.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-11, when thực hiện hợp lệ, then Tạo account_deletion_request -> đặt account.status=pending_deletion -> thu hồi mọi refresh token -> phát UserDeletionRequested -> các dịch vụ ẩn dữ liệu và xác nhận -> sau 30 ngày tiến trình purge xóa/ẩn danh dữ liệu -> phát UserDeletionCompleted.
- **AC02**: Kiểm soát được chứng minh: Xác thực lại; grace period; audit.
- **AC03**: Ca tối thiểu chạy thật: Ẩn nhanh; hủy; purge lặp; thiếu ack.
...
Ghi `TASK_ID=PH05-BE-ACC-11-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
