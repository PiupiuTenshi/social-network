# PH04-BE-COM-02-02 — Nghiệm thu COM-02 và bàn giao

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-COM-02.
- Mục tiêu duy nhất: Nghiệm thu COM-02 và bàn giao.
- Miền: community | Chức năng: COM-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-COM-02-02`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-COM-02-02
...
1. Rà soát COM-02 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Invite hết; join lặp; owner leave.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí COM-02 có report: Invite hết; join lặp; owner leave; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH04-BE-COM-02-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
