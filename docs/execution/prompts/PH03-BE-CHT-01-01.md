# PH03-BE-CHT-01-01 — Triển khai CHT-01: Tạo hội thoại trực tiếp hoặc nhóm

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-CHT-01.
- Mục tiêu duy nhất: Triển khai CHT-01: Tạo hội thoại trực tiếp hoặc nhóm.
- Miền: chat | Chức năng: CHT-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-CHT-01-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Trò chuyện. Hợp đồng tham chiếu: POST /conversations. Dữ liệu: conversation, members. Sự kiện: ConversationCreated tùy chọn.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-CHT-01-01
...
1. Theo hợp đồng đã khóa: Tạo hội thoại và thành viên; trả về DTO hội thoại.
2. Thực thi: Block policy; participant limits.
3. Khảo sát src/Services/Chat, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của CHT-01, when thực hiện hợp lệ, then Tạo hội thoại và thành viên; trả về DTO hội thoại.
- **AC02**: Kiểm soát được chứng minh: Block policy; participant limits.
- **AC03**: Ca tối thiểu chạy thật: DM trùng; member sai.
...
Ghi `TASK_ID=PH03-BE-CHT-01-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
