# PH04-BE-ACC-06-01 — Triển khai ACC-06: Cập nhật tham chiếu avatar

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-ACC-06.
- Mục tiêu duy nhất: Triển khai ACC-06: Cập nhật tham chiếu avatar.
- Miền: account | Chức năng: ACC-06.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-ACC-06-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: PATCH /users/me/avatar. Dữ liệu: profile. Sự kiện: UserAvatarUpdated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-ACC-06-01
...
1. Theo hợp đồng đã khóa: Gán avatar_media_id -> phát sự kiện cập nhật hồ sơ.
2. Thực thi: MediaReady; quyền sử dụng; If-Match.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-06, when thực hiện hợp lệ, then Gán avatar_media_id -> phát sự kiện cập nhật hồ sơ.
- **AC02**: Kiểm soát được chứng minh: MediaReady; quyền sử dụng; If-Match.
- **AC03**: Ca tối thiểu chạy thật: Media chưa sẵn sàng; ảnh không thuộc quyền.
...
Ghi `TASK_ID=PH04-BE-ACC-06-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
