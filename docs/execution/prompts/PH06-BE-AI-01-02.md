# PH06-BE-AI-01-02 — Nghiệm thu AI-01 và bàn giao

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-AI-01.
- Mục tiêu duy nhất: Nghiệm thu AI-01 và bàn giao.
- Miền: ai | Chức năng: AI-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-AI-01-02`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-AI-01-02
...
1. Rà soát AI-01 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Retry/DLQ; upsert idempotent.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí AI-01 có report: Retry/DLQ; upsert idempotent; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH06-BE-AI-01-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
