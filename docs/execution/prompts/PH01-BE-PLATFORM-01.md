# PH01-BE-PLATFORM-01 — BuildingBlocks, Gateway và quan sát cơ sở

## Mục tiêu và phạm vi

- Phase: PH01 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH01-BE-PLATFORM.
- Mục tiêu duy nhất: BuildingBlocks, Gateway và quan sát cơ sở.
...
- Nhánh đề xuất: `feat/PH01-BE-PLATFORM-01`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH01-BE-PLATFORM-01
...
1. Triển khai Outbox publisher, Inbox transaction, Idempotency-Key và schema envelope trong từng DB.
2. Gateway YARP, Problem Details, ETag, correlation, JWT biên, service JWT, Kafka ACL, CORS/rate limit.
3. Bổ sung trace RED, outbox age, lag, DLQ, retry bounded; mô phỏng broker/DB lỗi và crash sau publish.

## Tiêu chí chấp nhận

- **AC01**: Commit nghiệp vụ/outbox cùng giao dịch; phát lại cùng eventId không lặp tác dụng phụ.
- **AC02**: DB lỗi không phát dữ liệu ảo; Kafka down không làm mất sự kiện đã commit.
- **AC03**: BOLA checks ở dịch vụ; test contract lỗi, correlation, timeout/cancellation và kiến trúc đạt.
...
Ghi `TASK_ID=PH01-BE-PLATFORM-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
