# PH04-BE-MED-06-01 — Triển khai MED-06: Phương tiện cho tin nhắn thoại

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-MED-06.
- Mục tiêu duy nhất: Triển khai MED-06: Phương tiện cho tin nhắn thoại.
- Miền: media | Chức năng: MED-06.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-MED-06-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Phương tiện và trò chuyện. Hợp đồng tham chiếu: Media flow + Chat ref. Dữ liệu: media_asset + message ref. Sự kiện: MediaReady; TranscriptReady tùy chọn.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-MED-06-01
...
1. Theo hợp đồng đã khóa: Tải âm thanh qua luồng Phương tiện thông thường; message Trò chuyện chỉ tham chiếu MediaAsset Ready; tác vụ chuyển lời nói thành văn bản là tùy chọn.
2. Thực thi: Chat membership; duration.
3. Khảo sát src/Services/Media, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MED-06, when thực hiện hợp lệ, then Tải âm thanh qua luồng Phương tiện thông thường; message Trò chuyện chỉ tham chiếu MediaAsset Ready; tác vụ chuyển lời nói thành văn bản là tùy chọn.
- **AC02**: Kiểm soát được chứng minh: Chat membership; duration.
- **AC03**: Ca tối thiểu chạy thật: Transcription fail không xóa audio.
...
Ghi `TASK_ID=PH04-BE-MED-06-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
