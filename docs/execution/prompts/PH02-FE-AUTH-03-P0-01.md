# PH02-FE-AUTH-03-P0-01 — Triển khai AUTH-03 P0: Đăng ký

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: FE | Ưu tiên: P0 | Task cha: PH02-FE-AUTH-03-P0.
- Mục tiêu duy nhất: Triển khai AUTH-03 P0: Đăng ký.
- Miền: cross-cutting | Chức năng: ACC-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-FE-AUTH-03-P0-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /dang-ky. Vai trò: Khách. Chỉ bật ACC-01 tại P0. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH02-FE-AUTH-03-P0-01
...
1. Mục tiêu screen: Tạo tài khoản mới với email, tên người dùng và mật khẩu; cho phép chọn Google OAuth.
...
- **AC01**: Màn hình AUTH-03 tại /dang-ky thực hiện đúng quyền Khách và capability ACC-01.
- **AC02**: Trạng thái riêng được kiểm chứng: - Rỗng; - Kiểm tra tên người dùng; - Dữ liệu không hợp lệ; - Email hoặc tên đã tồn tại; - Thành công; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH02-FE-AUTH-03-P0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
