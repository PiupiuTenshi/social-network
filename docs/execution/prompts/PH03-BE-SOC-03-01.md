# PH03-BE-SOC-03-01 — Triển khai SOC-03: Tạo bài viết

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-SOC-03.
- Mục tiêu duy nhất: Triển khai SOC-03: Tạo bài viết.
- Miền: social | Chức năng: SOC-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-SOC-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội. Hợp đồng tham chiếu: POST /posts. Dữ liệu: post, media_ref, outbox. Sự kiện: PostCreated. P0 chỉ nội dung văn bản; từ chối media refs chưa hỗ trợ theo quyết định P0, mở ở P1.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-SOC-03-01
...
1. Theo hợp đồng đã khóa: Kiểm tra nội dung -> ghi Post, PostMediaRef và Outbox trong cùng giao dịch -> trả DTO đã tạo.
2. Thực thi: Nội dung; quyền media; Idempotency-Key.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của SOC-03, when thực hiện hợp lệ, then Kiểm tra nội dung -> ghi Post, PostMediaRef và Outbox trong cùng giao dịch -> trả DTO đã tạo.
- **AC02**: Kiểm soát được chứng minh: Nội dung; quyền media; Idempotency-Key.
- **AC03**: Ca tối thiểu chạy thật: Create lặp; media sai; Kafka down.
...
Ghi `TASK_ID=PH03-BE-SOC-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
