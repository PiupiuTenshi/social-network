# PH02-BE-ACC-07-01 — Triển khai ACC-07: Quyền riêng tư và thiết lập tài khoản

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH02-BE-ACC-07.
- Mục tiêu duy nhất: Triển khai ACC-07: Quyền riêng tư và thiết lập tài khoản.
- Miền: account | Chức năng: ACC-07.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-BE-ACC-07-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: PATCH /users/me/settings. Dữ liệu: account_setting. Sự kiện: Sự kiện khi cần.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH02-BE-ACC-07-01
...
1. Theo hợp đồng đã khóa: Cập nhật mức hiển thị và thiết lập bằng tập giá trị cho phép rõ ràng.
2. Thực thi: Chỉ self/admin; whitelist.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-07, when thực hiện hợp lệ, then Cập nhật mức hiển thị và thiết lập bằng tập giá trị cho phép rõ ràng.
- **AC02**: Kiểm soát được chứng minh: Chỉ self/admin; whitelist.
- **AC03**: Ca tối thiểu chạy thật: Enum sai; mass assignment.
...
Ghi `TASK_ID=PH02-BE-ACC-07-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
