# PH03-BE-SOC-04-01 — Triển khai SOC-04: Sửa bài viết

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-SOC-04.
- Mục tiêu duy nhất: Triển khai SOC-04: Sửa bài viết.
- Miền: social | Chức năng: SOC-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-SOC-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội. Hợp đồng tham chiếu: PATCH /posts/{id}. Dữ liệu: post. Sự kiện: PostUpdated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-SOC-04-01
...
1. Theo hợp đồng đã khóa: Kiểm tra version -> cập nhật các trường được phép -> ghi Outbox PostUpdated.
2. Thực thi: Owner; If-Match; sanitize.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của SOC-04, when thực hiện hợp lệ, then Kiểm tra version -> cập nhật các trường được phép -> ghi Outbox PostUpdated.
- **AC02**: Kiểm soát được chứng minh: Owner; If-Match; sanitize.
- **AC03**: Ca tối thiểu chạy thật: Stale trả 412; không phải owner.
...
Ghi `TASK_ID=PH03-BE-SOC-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
