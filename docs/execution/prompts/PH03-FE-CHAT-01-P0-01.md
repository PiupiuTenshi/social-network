# PH03-FE-CHAT-01-P0-01 — Triển khai CHAT-01 P0: Danh sách hội thoại

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: FE | Ưu tiên: P0 | Task cha: PH03-FE-CHAT-01-P0.
- Mục tiêu duy nhất: Triển khai CHAT-01 P0: Danh sách hội thoại.
- Miền: cross-cutting | Chức năng: CHT-01, CHT-08.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-FE-CHAT-01-P0-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /tin-nhan. Vai trò: Người dùng. Chỉ bật CHT-01, CHT-08 tại P0. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-FE-CHAT-01-P0-01
...
1. Mục tiêu screen: Hiển thị DM/nhóm theo hoạt động gần nhất, chưa đọc và trạng thái hiện diện.
...
- **AC01**: Màn hình CHAT-01 tại /tin-nhan thực hiện đúng quyền Người dùng và capability CHT-01, CHT-08.
- **AC02**: Trạng thái riêng được kiểm chứng: - Có dữ liệu; - Rỗng; - Có tin chưa đọc; - Ngoại tuyến; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH03-FE-CHAT-01-P0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
