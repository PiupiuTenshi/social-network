# PH04-BE-MED-03-01 — Triển khai MED-03: Hoàn tất và xác minh tải lên

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-MED-03.
- Mục tiêu duy nhất: Triển khai MED-03: Hoàn tất và xác minh tải lên.
- Miền: media | Chức năng: MED-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-MED-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Phương tiện. Hợp đồng tham chiếu: POST complete. Dữ liệu: session, media_asset, outbox. Sự kiện: MediaUploaded.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-MED-03-01
...
1. Theo hợp đồng đã khóa: Gọi HEAD/stat đối tượng -> xác minh tồn tại, kích thước và loại -> chuyển sang Uploaded -> ghi Outbox.
2. Thực thi: HEAD/stat; owner; Idempotency-Key.
3. Khảo sát src/Services/Media, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MED-03, when thực hiện hợp lệ, then Gọi HEAD/stat đối tượng -> xác minh tồn tại, kích thước và loại -> chuyển sang Uploaded -> ghi Outbox.
- **AC02**: Kiểm soát được chứng minh: HEAD/stat; owner; Idempotency-Key.
- **AC03**: Ca tối thiểu chạy thật: Claim giả; complete lặp.
...
Ghi `TASK_ID=PH04-BE-MED-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
