# PH01-FE-FOUNDATION-02 — HTTP/auth/realtime adapters có xử lý lỗi và vòng đời

## Mục tiêu và phạm vi

- Phase: PH01 | Nhóm: FE | Ưu tiên: P0 | Task cha: PH01-FE-FOUNDATION.
- Mục tiêu duy nhất: HTTP/auth/realtime adapters có xử lý lỗi và vòng đời.
...
- Nhánh đề xuất: `feat/PH01-FE-FOUNDATION-02`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH01-FE-FOUNDATION-02
...
1. Sinh type từ hợp đồng P0 đã khóa; auth refresh coordinator chỉ một lần, không loop.
2. Map Problem Details/412/429, hủy request, response cũ; SignalR adapter tách LiveKit.

## Tiêu chí chấp nhận

- **AC01**: Refresh đồng thời không tạo nhiều rotation; không lộ token trong URL/log/storage ngoài policy.
- **AC02**: Retry giữ nội dung và khóa idempotency; mock chỉ dev/test; có component/service tests.
...
Ghi `TASK_ID=PH01-FE-FOUNDATION-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
