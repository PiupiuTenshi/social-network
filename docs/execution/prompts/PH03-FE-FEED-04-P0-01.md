# PH03-FE-FEED-04-P0-01 — Triển khai FEED-04 P0: Sửa bài viết

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: FE | Ưu tiên: P0 | Task cha: PH03-FE-FEED-04-P0.
- Mục tiêu duy nhất: Triển khai FEED-04 P0: Sửa bài viết.
- Miền: cross-cutting | Chức năng: SOC-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-FE-FEED-04-P0-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /bai-viet/:id/sua. Vai trò: Chủ sở hữu. Chỉ bật SOC-04 tại P0. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-FE-FEED-04-P0-01
...
1. Mục tiêu screen: Cập nhật nội dung hoặc phạm vi hiển thị bằng If-Match để tránh ghi đè dữ liệu mới.
...
- **AC01**: Màn hình FEED-04 tại /bai-viet/:id/sua thực hiện đúng quyền Chủ sở hữu và capability SOC-04.
- **AC02**: Trạng thái riêng được kiểm chứng: - Mặc định; - Đang lưu; - Xung đột phiên bản 412; - Không có quyền; - Bài viết đã xóa; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH03-FE-FEED-04-P0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
