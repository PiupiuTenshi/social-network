# PH04-BE-COM-02-01 — Triển khai COM-02: Mời, tham gia và rời cộng đồng

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-COM-02.
- Mục tiêu duy nhất: Triển khai COM-02: Mời, tham gia và rời cộng đồng.
- Miền: community | Chức năng: COM-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-COM-02-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Cộng đồng. Hợp đồng tham chiếu: invite/join/leave. Dữ liệu: invite, community_member. Sự kiện: MemberJoined/LeftCommunity.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-COM-02-01
...
1. Theo hợp đồng đã khóa: Tạo lời mời; tham gia thì tạo membership; rời đi thì chuyển membership sang không hoạt động.
2. Thực thi: Hết hạn; max uses; ban; transfer owner.
3. Khảo sát src/Services/Community, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của COM-02, when thực hiện hợp lệ, then Tạo lời mời; tham gia thì tạo membership; rời đi thì chuyển membership sang không hoạt động.
- **AC02**: Kiểm soát được chứng minh: Hết hạn; max uses; ban; transfer owner.
- **AC03**: Ca tối thiểu chạy thật: Invite hết; join lặp; owner leave.
...
Ghi `TASK_ID=PH04-BE-COM-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
