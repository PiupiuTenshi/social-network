# PH04-DATA-MED-P1-01 — Thiết kế lược đồ MED P1

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: DATA | Ưu tiên: P1 | Task cha: PH04-DATA-MED-P1.
- Mục tiêu duy nhất: Thiết kế lược đồ MED P1.
- Miền: media | Chức năng: nền tảng/quy trình.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-DATA-MED-P1-01`; chưa được tạo chỉ vì có trong prompt.
- UploadSession/MediaAsset, object metadata; state, expiry, reference/retention_hold và cleanup

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-DATA-MED-P1-01
...
1. Cụ thể hóa UploadSession/MediaAsset, object metadata; state, expiry, reference/retention_hold và cleanup.
2. Đối chiếu ERD nguồn và hợp đồng đã khóa; chọn index từ query, mapping UTC/version, data classification.

## Tiêu chí chấp nhận

- **AC01**: ERD và mapping cụ thể cho UploadSession/MediaAsset, object metadata; state, expiry, reference/retention_hold và cleanup; không lấn ownership.
...
Ghi `TASK_ID=PH04-DATA-MED-P1-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
