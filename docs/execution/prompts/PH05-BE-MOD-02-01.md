# PH05-BE-MOD-02-01 — Triển khai MOD-02: Duyệt hàng đợi báo cáo

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-MOD-02.
- Mục tiêu duy nhất: Triển khai MOD-02: Duyệt hàng đợi báo cáo.
- Miền: social | Chức năng: MOD-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH05-BE-MOD-02-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội - khối điều hành. Hợp đồng tham chiếu: GET /moderation/reports. Dữ liệu: content_report. Sự kiện: Không.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-MOD-02-01
...
1. Theo hợp đồng đã khóa: Lọc theo status/priority/type -> lấy bằng con trỏ -> kiểm tra phạm vi -> trả dữ liệu đã che trường nhạy cảm -> ghi audit truy cập.
2. Thực thi: Moderator role; field masking.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MOD-02, when thực hiện hợp lệ, then Lọc theo status/priority/type -> lấy bằng con trỏ -> kiểm tra phạm vi -> trả dữ liệu đã che trường nhạy cảm -> ghi audit truy cập.
- **AC02**: Kiểm soát được chứng minh: Moderator role; field masking.
- **AC03**: Ca tối thiểu chạy thật: Người thường bị 403; cursor/filter.
...
Ghi `TASK_ID=PH05-BE-MOD-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
