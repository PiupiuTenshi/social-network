...
```powershell
python -X utf8 scripts/task_gate.py check PH03-DATA-CHT-P0-01
...
1. Cụ thể hóa conversation, conversation_member, message, message_reaction, notification; unique clientMessageId và read cursor tăng.
2. Đối chiếu ERD nguồn và hợp đồng đã khóa; chọn index từ query, mapping UTC/version, data classification.

## Tiêu chí chấp nhận

- **AC01**: ERD và mapping cụ thể cho conversation, conversation_member, message, message_reaction, notification; unique clientMessageId và read cursor tăng; không lấn ownership.
...
Ghi `TASK_ID=PH03-DATA-CHT-P0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
