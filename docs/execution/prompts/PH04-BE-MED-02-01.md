# PH04-BE-MED-02-01 — Triển khai MED-02: Tải trực tiếp

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-MED-02.
- Mục tiêu duy nhất: Triển khai MED-02: Tải trực tiếp.
- Miền: media | Chức năng: MED-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-MED-02-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mặt phẳng SeaweedFS. Hợp đồng tham chiếu: PUT URL ký trước. Dữ liệu: Object storage. Sự kiện: Không. Task BE kiểm chứng capability/adapter/hạ tầng; bytes/track đi trực tiếp từ client, không tạo endpoint chuyển tiếp media.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-MED-02-01
...
1. Theo hợp đồng đã khóa: Máy khách gửi dữ liệu bằng PUT trực tiếp tới kho đối tượng.
2. Thực thi: Signature, TTL, key constraints.
3. Khảo sát src/Services/Media, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MED-02, when thực hiện hợp lệ, then Máy khách gửi dữ liệu bằng PUT trực tiếp tới kho đối tượng.
- **AC02**: Kiểm soát được chứng minh: Signature, TTL, key constraints.
- **AC03**: Ca tối thiểu chạy thật: Hết hạn; backend không nhận bytes.
...
Ghi `TASK_ID=PH04-BE-MED-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
