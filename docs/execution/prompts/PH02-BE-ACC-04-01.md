# PH02-BE-ACC-04-01 — Triển khai ACC-04: Đăng xuất

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH02-BE-ACC-04.
- Mục tiêu duy nhất: Triển khai ACC-04: Đăng xuất.
- Miền: account | Chức năng: ACC-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-BE-ACC-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: POST /auth/logout. Dữ liệu: refresh_token. Sự kiện: Không bắt buộc.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH02-BE-ACC-04-01
...
1. Theo hợp đồng đã khóa: Thu hồi refresh token được yêu cầu hoặc toàn bộ họ phiên.
2. Thực thi: Đúng chủ sở hữu; lặp an toàn.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-04, when thực hiện hợp lệ, then Thu hồi refresh token được yêu cầu hoặc toàn bộ họ phiên.
- **AC02**: Kiểm soát được chứng minh: Đúng chủ sở hữu; lặp an toàn.
- **AC03**: Ca tối thiểu chạy thật: Đăng xuất lặp; token đã thu hồi.
...
Ghi `TASK_ID=PH02-BE-ACC-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
