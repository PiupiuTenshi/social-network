# PH05-BE-MOD-03-01 — Triển khai MOD-03: Áp dụng hành động điều hành

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-MOD-03.
- Mục tiêu duy nhất: Triển khai MOD-03: Áp dụng hành động điều hành.
- Miền: social | Chức năng: MOD-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH05-BE-MOD-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội điều phối; dịch vụ nguồn áp dụng. Hợp đồng tham chiếu: POST /reports/{id}/actions. Dữ liệu: moderation_action, outbox. Sự kiện: ModerationActionRequested.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-MOD-03-01
...
1. Theo hợp đồng đã khóa: Ghi moderation_action bất biến + Outbox ModerationActionRequested -> dịch vụ nguồn kiểm tra/áp dụng lặp an toàn -> phát Completed hoặc Rejected -> cập nhật report.
2. Thực thi: Role/hierarchy; reason; append-only.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MOD-03, when thực hiện hợp lệ, then Ghi moderation_action bất biến + Outbox ModerationActionRequested -> dịch vụ nguồn kiểm tra/áp dụng lặp an toàn -> phát Completed hoặc Rejected -> cập nhật report.
- **AC02**: Kiểm soát được chứng minh: Role/hierarchy; reason; append-only.
- **AC03**: Ca tối thiểu chạy thật: Action lặp; target service reject.
...
Ghi `TASK_ID=PH05-BE-MOD-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
