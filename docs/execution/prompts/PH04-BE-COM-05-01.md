# PH04-BE-COM-05-01 — Triển khai COM-05: Điều hành thành viên

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-COM-05.
- Mục tiêu duy nhất: Triển khai COM-05: Điều hành thành viên.
- Miền: community | Chức năng: COM-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-COM-05-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Cộng đồng. Hợp đồng tham chiếu: POST member moderation. Dữ liệu: community_member. Sự kiện: MemberModerated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-COM-05-01
...
1. Theo hợp đồng đã khóa: Tắt tiếng, loại hoặc cấm theo chính sách; sau đó phát sự kiện.
2. Thực thi: Role hierarchy; audit.
3. Khảo sát src/Services/Community, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của COM-05, when thực hiện hợp lệ, then Tắt tiếng, loại hoặc cấm theo chính sách; sau đó phát sự kiện.
- **AC02**: Kiểm soát được chứng minh: Role hierarchy; audit.
- **AC03**: Ca tối thiểu chạy thật: Mute/kick/ban; target cao hơn.
...
Ghi `TASK_ID=PH04-BE-COM-05-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
