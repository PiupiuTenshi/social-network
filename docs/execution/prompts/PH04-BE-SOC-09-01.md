# PH04-BE-SOC-09-01 — Triển khai SOC-09: Tạo bài viết video

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-SOC-09.
- Mục tiêu duy nhất: Triển khai SOC-09: Tạo bài viết video.
- Miền: social | Chức năng: SOC-09.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-SOC-09-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội. Hợp đồng tham chiếu: POST /posts type=video. Dữ liệu: post, media_ref. Sự kiện: PostCreated, PostUpdated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-SOC-09-01
...
1. Theo hợp đồng đã khóa: Tạo post với type=video và visibility=processing -> Phương tiện chuyển mã và tạo ảnh thu nhỏ bất đồng bộ -> khi nhận MediaReady, Mạng xã hội chuyển visibility sang published. SOC-03 với type=content được hiển thị ngay.
2. Thực thi: Media pipeline; owner-only khi processing.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của SOC-09, when thực hiện hợp lệ, then Tạo post với type=video và visibility=processing -> Phương tiện chuyển mã và tạo ảnh thu nhỏ bất đồng bộ -> khi nhận MediaReady, Mạng xã hội chuyển visibility sang published. SOC-03 với type=content được hiển thị ngay.
- **AC02**: Kiểm soát được chứng minh: Media pipeline; owner-only khi processing.
- **AC03**: Ca tối thiểu chạy thật: Ready/Failed; không vào bảng tin sớm.
...
Ghi `TASK_ID=PH04-BE-SOC-09-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
