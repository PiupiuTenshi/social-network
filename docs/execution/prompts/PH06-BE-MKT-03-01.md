# PH06-BE-MKT-03-01 — Triển khai MKT-03: Cập nhật và giữ tồn kho

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-MKT-03.
- Mục tiêu duy nhất: Triển khai MKT-03: Cập nhật và giữ tồn kho.
- Miền: commerce | Chức năng: MKT-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-MKT-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Thương mại. Hợp đồng tham chiếu: PUT inventory; checkout. Dữ liệu: inventory. Sự kiện: Reserved/Released.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-MKT-03-01
...
1. Theo hợp đồng đã khóa: Người bán điều chỉnh tồn kho; bước thanh toán giữ số lượng bằng thao tác nguyên tử.
2. Thực thi: Atomic update/row lock; non-negative.
3. Khảo sát src/Services/Commerce, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MKT-03, when thực hiện hợp lệ, then Người bán điều chỉnh tồn kho; bước thanh toán giữ số lượng bằng thao tác nguyên tử.
- **AC02**: Kiểm soát được chứng minh: Atomic update/row lock; non-negative.
- **AC03**: Ca tối thiểu chạy thật: Hai buyer không oversell.
...
Ghi `TASK_ID=PH06-BE-MKT-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
