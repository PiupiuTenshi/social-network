# PH03-BE-SOC-02-01 — Triển khai SOC-02: Chặn và bỏ chặn

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-SOC-02.
- Mục tiêu duy nhất: Triển khai SOC-02: Chặn và bỏ chặn.
- Miền: social | Chức năng: SOC-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-SOC-02-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội. Hợp đồng tham chiếu: POST/DELETE /users/{id}/block. Dữ liệu: block, follow. Sự kiện: UserBlocked/Unblocked.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-SOC-02-01
...
1. Theo hợp đồng đã khóa: Tạo quan hệ block, xóa các quan hệ follow không còn hợp lệ và phát sự kiện.
2. Thực thi: Cấm self; dọn follow; lặp an toàn.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của SOC-02, when thực hiện hợp lệ, then Tạo quan hệ block, xóa các quan hệ follow không còn hợp lệ và phát sự kiện.
- **AC02**: Kiểm soát được chứng minh: Cấm self; dọn follow; lặp an toàn.
- **AC03**: Ca tối thiểu chạy thật: Block race; visibility sau block.
...
Ghi `TASK_ID=PH03-BE-SOC-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
