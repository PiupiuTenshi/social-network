# PH05-BE-MOD-01-01 — Triển khai MOD-01: Gửi báo cáo vi phạm

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-MOD-01.
- Mục tiêu duy nhất: Triển khai MOD-01: Gửi báo cáo vi phạm.
- Miền: social | Chức năng: MOD-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH05-BE-MOD-01-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội - khối điều hành. Hợp đồng tham chiếu: POST /reports. Dữ liệu: content_report, evidence. Sự kiện: ContentReported.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-MOD-01-01
...
1. Theo hợp đồng đã khóa: Xác minh target qua contract/projection -> kiểm tra giới hạn -> tạo content_report và evidence tối thiểu -> Outbox ContentReported -> trả mã báo cáo.
2. Thực thi: Target visible; rate limit; dedupe.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MOD-01, when thực hiện hợp lệ, then Xác minh target qua contract/projection -> kiểm tra giới hạn -> tạo content_report và evidence tối thiểu -> Outbox ContentReported -> trả mã báo cáo.
- **AC02**: Kiểm soát được chứng minh: Target visible; rate limit; dedupe.
- **AC03**: Ca tối thiểu chạy thật: Target sai; report spam; snapshot tối thiểu.
...
Ghi `TASK_ID=PH05-BE-MOD-01-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
