# PH05-BE-DELETION-01 — Xóa tài khoản: Ẩn/ẩn danh post/comment, dọn follow/block và bảo vệ retention_hold

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-DELETION.
- Mục tiêu duy nhất: Xóa tài khoản: Ẩn/ẩn danh post/comment, dọn follow/block và bảo vệ retention_hold.
...
- Nhánh đề xuất: `feat/PH05-BE-DELETION-01`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-DELETION-01
...
1. Owner xử lý: social; Ẩn/ẩn danh post/comment, dọn follow/block và bảo vệ retention_hold.
2. Chỉ qua contract/event; ghi ack cùng transaction, lặp an toàn và có metric.

## Tiêu chí chấp nhận

- **AC01**: Ẩn/ẩn danh post/comment, dọn follow/block và bảo vệ retention_hold có bằng chứng test và artifact được đối soát.
...
Ghi `TASK_ID=PH05-BE-DELETION-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
