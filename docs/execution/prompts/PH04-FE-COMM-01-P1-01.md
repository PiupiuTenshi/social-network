# PH04-FE-COMM-01-P1-01 — Triển khai COMM-01 P1: Khám phá cộng đồng

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: FE | Ưu tiên: P1 | Task cha: PH04-FE-COMM-01-P1.
- Mục tiêu duy nhất: Triển khai COMM-01 P1: Khám phá cộng đồng.
- Miền: cross-cutting | Chức năng: COM-01, COM-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-FE-COMM-01-P1-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /cong-dong. Vai trò: Người dùng. Chỉ bật COM-01, COM-02 tại P1. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-FE-COMM-01-P1-01
...
1. Mục tiêu screen: Tìm cộng đồng công khai, xem gợi ý và tạo cộng đồng mới.
...
- **AC01**: Màn hình COMM-01 tại /cong-dong thực hiện đúng quyền Người dùng và capability COM-01, COM-02.
- **AC02**: Trạng thái riêng được kiểm chứng: - Có gợi ý; - Không có kết quả; - Đang tham gia; - Bị cấm tham gia; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH04-FE-COMM-01-P1-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
