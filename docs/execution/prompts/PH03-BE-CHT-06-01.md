# PH03-BE-CHT-06-01 — Triển khai CHT-06: Trạng thái hiện diện

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-CHT-06.
- Mục tiêu duy nhất: Triển khai CHT-06: Trạng thái hiện diện.
- Miền: chat | Chức năng: CHT-06.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-CHT-06-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Trò chuyện/SignalR. Hợp đồng tham chiếu: SignalR connect/disconnect. Dữ liệu: Valkey presence. Sự kiện: PresenceChanged.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-CHT-06-01
...
1. Theo hợp đồng đã khóa: Theo dõi số kết nối và lần hoạt động gần nhất trong Valkey với TTL; phát theo chính sách riêng tư.
2. Thực thi: Nhiều thiết bị; privacy.
3. Khảo sát src/Services/Chat, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của CHT-06, when thực hiện hợp lệ, then Theo dõi số kết nối và lần hoạt động gần nhất trong Valkey với TTL; phát theo chính sách riêng tư.
- **AC02**: Kiểm soát được chứng minh: Nhiều thiết bị; privacy.
- **AC03**: Ca tối thiểu chạy thật: Disconnect bất thường; TTL.
...
Ghi `TASK_ID=PH03-BE-CHT-06-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
