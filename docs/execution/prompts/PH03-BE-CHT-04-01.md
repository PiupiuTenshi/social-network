# PH03-BE-CHT-04-01 — Triển khai CHT-04: Tương tác với tin nhắn

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-CHT-04.
- Mục tiêu duy nhất: Triển khai CHT-04: Tương tác với tin nhắn.
- Miền: chat | Chức năng: CHT-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-CHT-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Trò chuyện. Hợp đồng tham chiếu: PUT/DELETE message reaction. Dữ liệu: message_reaction. Sự kiện: SignalR ReactionChanged.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-CHT-04-01
...
1. Theo hợp đồng đã khóa: Thêm, cập nhật hoặc xóa tương tác bằng ràng buộc duy nhất.
2. Thực thi: Membership; unique; rate limit.
3. Khảo sát src/Services/Chat, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của CHT-04, when thực hiện hợp lệ, then Thêm, cập nhật hoặc xóa tương tác bằng ràng buộc duy nhất.
- **AC02**: Kiểm soát được chứng minh: Membership; unique; rate limit.
- **AC03**: Ca tối thiểu chạy thật: Thêm/xóa lặp.
...
Ghi `TASK_ID=PH03-BE-CHT-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
