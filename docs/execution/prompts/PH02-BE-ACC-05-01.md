# PH02-BE-ACC-05-01 — Triển khai ACC-05: Xem và cập nhật hồ sơ

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH02-BE-ACC-05.
- Mục tiêu duy nhất: Triển khai ACC-05: Xem và cập nhật hồ sơ.
- Miền: account | Chức năng: ACC-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-BE-ACC-05-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: GET/PATCH /users. Dữ liệu: profile. Sự kiện: UserProfileUpdated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH02-BE-ACC-05-01
...
1. Theo hợp đồng đã khóa: GET đọc hồ sơ hoặc PATCH các trường cho phép -> kiểm tra version -> cập nhật profile -> ghi Outbox UserProfileUpdated.
2. Thực thi: Quyền riêng tư; If-Match.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-05, when thực hiện hợp lệ, then GET đọc hồ sơ hoặc PATCH các trường cho phép -> kiểm tra version -> cập nhật profile -> ghi Outbox UserProfileUpdated.
- **AC02**: Kiểm soát được chứng minh: Quyền riêng tư; If-Match.
- **AC03**: Ca tối thiểu chạy thật: Ẩn hồ sơ; cập nhật stale trả 412.
...
Ghi `TASK_ID=PH02-BE-ACC-05-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
