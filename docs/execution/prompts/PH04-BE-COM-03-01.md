# PH04-BE-COM-03-01 — Triển khai COM-03: Vai trò và quyền

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-COM-03.
- Mục tiêu duy nhất: Triển khai COM-03: Vai trò và quyền.
- Miền: community | Chức năng: COM-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-COM-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Cộng đồng. Hợp đồng tham chiếu: roles/member roles. Dữ liệu: role, member_role. Sự kiện: RoleUpdated/PermissionUpdated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-COM-03-01
...
1. Theo hợp đồng đã khóa: Tạo hoặc cập nhật vai trò, gán cho thành viên và tính quyền hiệu lực.
2. Thực thi: Không leo quyền; owner protected.
3. Khảo sát src/Services/Community, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của COM-03, when thực hiện hợp lệ, then Tạo hoặc cập nhật vai trò, gán cho thành viên và tính quyền hiệu lực.
- **AC02**: Kiểm soát được chứng minh: Không leo quyền; owner protected.
- **AC03**: Ca tối thiểu chạy thật: Escalation bị chặn; cache invalidation.
...
Ghi `TASK_ID=PH04-BE-COM-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
