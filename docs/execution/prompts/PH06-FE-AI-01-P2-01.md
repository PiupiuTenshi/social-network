# PH06-FE-AI-01-P2-01 — Triển khai AI-01 P2: Trợ lý AI

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: FE | Ưu tiên: P2 | Task cha: PH06-FE-AI-01-P2.
- Mục tiêu duy nhất: Triển khai AI-01 P2: Trợ lý AI.
- Miền: cross-cutting | Chức năng: AI-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-FE-AI-01-P2-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /tro-ly. Vai trò: Người dùng. Chỉ bật AI-03 tại P2. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-FE-AI-01-P2-01
...
1. Mục tiêu screen: Hỏi đáp dựa trên nguồn người dùng được phép truy cập, stream câu trả lời và hiển thị nguồn tham chiếu.
...
- **AC01**: Màn hình AI-01 tại /tro-ly thực hiện đúng quyền Người dùng và capability AI-03.
- **AC02**: Trạng thái riêng được kiểm chứng: - Rỗng; - Đang truy xuất; - Đang stream; - Đã hoàn tất; - AI không khả dụng; - Bị giới hạn tần suất; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH06-FE-AI-01-P2-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
