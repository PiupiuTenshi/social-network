# PH06-BE-AI-02-01 — Triển khai AI-02: Tìm kiếm ngữ nghĩa

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-AI-02.
- Mục tiêu duy nhất: Triển khai AI-02: Tìm kiếm ngữ nghĩa.
- Miền: ai | Chức năng: AI-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-AI-02-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: AI và tìm kiếm. Hợp đồng tham chiếu: GET /search/semantic. Dữ liệu: pgvector. Sự kiện: Không.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-AI-02-01
...
1. Theo hợp đồng đã khóa: Tạo embedding cho truy vấn -> tính độ tương đồng bằng pgvector -> lọc theo phạm vi được cấp quyền -> trả tham chiếu và đoạn trích đã xếp hạng.
2. Thực thi: Authorization trước exposure.
3. Khảo sát src/Services/AiSearch, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của AI-02, when thực hiện hợp lệ, then Tạo embedding cho truy vấn -> tính độ tương đồng bằng pgvector -> lọc theo phạm vi được cấp quyền -> trả tham chiếu và đoạn trích đã xếp hạng.
- **AC02**: Kiểm soát được chứng minh: Authorization trước exposure.
- **AC03**: Ca tối thiểu chạy thật: AI down -> FTS; private filter.
...
Ghi `TASK_ID=PH06-BE-AI-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
