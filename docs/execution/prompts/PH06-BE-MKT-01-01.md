# PH06-BE-MKT-01-01 — Triển khai MKT-01: Tạo và sửa tin đăng

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-MKT-01.
- Mục tiêu duy nhất: Triển khai MKT-01: Tạo và sửa tin đăng.
- Miền: commerce | Chức năng: MKT-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-MKT-01-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Thương mại. Hợp đồng tham chiếu: POST/PATCH /listings. Dữ liệu: listing, media_ref. Sự kiện: ListingCreated/Updated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-MKT-01-01
...
1. Theo hợp đồng đã khóa: Tạo hoặc cập nhật listing với tham chiếu Phương tiện, giá, trạng thái và version.
2. Thực thi: Seller owner; price; If-Match.
3. Khảo sát src/Services/Commerce, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MKT-01, when thực hiện hợp lệ, then Tạo hoặc cập nhật listing với tham chiếu Phương tiện, giá, trạng thái và version.
- **AC02**: Kiểm soát được chứng minh: Seller owner; price; If-Match.
- **AC03**: Ca tối thiểu chạy thật: Stale 412; media sai.
...
Ghi `TASK_ID=PH06-BE-MKT-01-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
