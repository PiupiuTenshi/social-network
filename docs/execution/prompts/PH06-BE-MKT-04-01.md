# PH06-BE-MKT-04-01 — Triển khai MKT-04: Tạo đơn hàng

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-MKT-04.
- Mục tiêu duy nhất: Triển khai MKT-04: Tạo đơn hàng.
- Miền: commerce | Chức năng: MKT-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-MKT-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Thương mại. Hợp đồng tham chiếu: POST /orders. Dữ liệu: order, items, inventory. Sự kiện: OrderCreated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-MKT-04-01
...
1. Theo hợp đồng đã khóa: Tạo bản chụp đơn hàng -> giữ tồn kho -> chuyển sang AwaitingPayment.
2. Thực thi: Idempotency-Key; price snapshot.
3. Khảo sát src/Services/Commerce, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MKT-04, when thực hiện hợp lệ, then Tạo bản chụp đơn hàng -> giữ tồn kho -> chuyển sang AwaitingPayment.
- **AC02**: Kiểm soát được chứng minh: Idempotency-Key; price snapshot.
- **AC03**: Ca tối thiểu chạy thật: Stock fail không ghost order.
...
Ghi `TASK_ID=PH06-BE-MKT-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
