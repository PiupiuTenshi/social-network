# PH04-BE-RTC-03-02 — Nghiệm thu RTC-03 và bàn giao

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-RTC-03.
- Mục tiêu duy nhất: Nghiệm thu RTC-03 và bàn giao.
- Miền: community | Chức năng: RTC-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-RTC-03-02`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-RTC-03-02
...
1. Rà soát RTC-03 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Mạng yếu; giới hạn room.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí RTC-03 có report: Mạng yếu; giới hạn room; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH04-BE-RTC-03-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
