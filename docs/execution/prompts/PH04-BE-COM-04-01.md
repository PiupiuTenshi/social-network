# PH04-BE-COM-04-01 — Triển khai COM-04: Tạo kênh văn bản hoặc kênh thoại

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-COM-04.
- Mục tiêu duy nhất: Triển khai COM-04: Tạo kênh văn bản hoặc kênh thoại.
- Miền: community | Chức năng: COM-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-COM-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Cộng đồng. Hợp đồng tham chiếu: POST /communities/{id}/channels. Dữ liệu: channel, outbox. Sự kiện: ChannelCreated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-COM-04-01
...
1. Theo hợp đồng đã khóa: Tạo siêu dữ liệu kênh và phát sự kiện; Trò chuyện tạo bản chiếu cho kênh văn bản.
2. Thực thi: ManageChannel; type-specific.
3. Khảo sát src/Services/Community, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của COM-04, when thực hiện hợp lệ, then Tạo siêu dữ liệu kênh và phát sự kiện; Trò chuyện tạo bản chiếu cho kênh văn bản.
- **AC02**: Kiểm soát được chứng minh: ManageChannel; type-specific.
- **AC03**: Ca tối thiểu chạy thật: Projection Trò chuyện eventual.
...
Ghi `TASK_ID=PH04-BE-COM-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
