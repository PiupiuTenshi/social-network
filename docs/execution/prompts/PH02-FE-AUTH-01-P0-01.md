# PH02-FE-AUTH-01-P0-01 — Triển khai AUTH-01 P0: Chào mừng

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: FE | Ưu tiên: P0 | Task cha: PH02-FE-AUTH-01-P0.
- Mục tiêu duy nhất: Triển khai AUTH-01 P0: Chào mừng.
- Miền: cross-cutting | Chức năng: ACC-01, ACC-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-FE-AUTH-01-P0-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /. Vai trò: Khách. Chỉ bật ACC-01, ACC-02 tại P0. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH02-FE-AUTH-01-P0-01
...
1. Mục tiêu screen: Giới thiệu ngắn gọn giá trị sản phẩm và dẫn người dùng đến đăng nhập hoặc đăng ký.
...
- **AC01**: Màn hình AUTH-01 tại / thực hiện đúng quyền Khách và capability ACC-01, ACC-02.
- **AC02**: Trạng thái riêng được kiểm chứng: - Mặc định; - Mạng chậm; - Bảo trì có kiểm soát; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH02-FE-AUTH-01-P0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
