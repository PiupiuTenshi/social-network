# PH06-BE-MKT-07-01 — Triển khai MKT-07: Phương tiện của tin đăng

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-MKT-07.
- Mục tiêu duy nhất: Triển khai MKT-07: Phương tiện của tin đăng.
- Miền: commerce | Chức năng: MKT-07.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-MKT-07-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Commerce + Phương tiện. Hợp đồng tham chiếu: Listing media refs. Dữ liệu: listing_media_ref. Sự kiện: Không.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-MKT-07-01
...
1. Theo hợp đồng đã khóa: Lưu tham chiếu MediaId trong listing; không sao chép dữ liệu nhị phân.
2. Thực thi: MediaReady; ownership/purpose.
3. Khảo sát src/Services/Commerce, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MKT-07, when thực hiện hợp lệ, then Lưu tham chiếu MediaId trong listing; không sao chép dữ liệu nhị phân.
- **AC02**: Kiểm soát được chứng minh: MediaReady; ownership/purpose.
- **AC03**: Ca tối thiểu chạy thật: Shared media không bị xóa sớm.
...
Ghi `TASK_ID=PH06-BE-MKT-07-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
