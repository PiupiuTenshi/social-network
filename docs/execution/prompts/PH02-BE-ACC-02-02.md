# PH02-BE-ACC-02-02 — Nghiệm thu ACC-02 và bàn giao

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH02-BE-ACC-02.
- Mục tiêu duy nhất: Nghiệm thu ACC-02 và bàn giao.
- Miền: account | Chức năng: ACC-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-BE-ACC-02-02`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH02-BE-ACC-02-02
...
1. Rà soát ACC-02 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Sai mật khẩu, khóa tạm, token claims.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí ACC-02 có report: Sai mật khẩu, khóa tạm, token claims; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH02-BE-ACC-02-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
