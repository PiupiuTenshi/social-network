# PH02-BE-ACC-02-01 — Triển khai ACC-02: Đăng nhập

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH02-BE-ACC-02.
- Mục tiêu duy nhất: Triển khai ACC-02: Đăng nhập.
- Miền: account | Chức năng: ACC-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-BE-ACC-02-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: POST /auth/login. Dữ liệu: refresh_token. Sự kiện: Không bắt buộc.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH02-BE-ACC-02-01
...
1. Theo hợp đồng đã khóa: Xác minh thông tin đăng nhập -> cấp access token sống ngắn và refresh token -> lưu hash của refresh token.
2. Thực thi: Rate limit; tài khoản active; không lộ email tồn tại.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-02, when thực hiện hợp lệ, then Xác minh thông tin đăng nhập -> cấp access token sống ngắn và refresh token -> lưu hash của refresh token.
- **AC02**: Kiểm soát được chứng minh: Rate limit; tài khoản active; không lộ email tồn tại.
- **AC03**: Ca tối thiểu chạy thật: Sai mật khẩu, khóa tạm, token claims.
...
Ghi `TASK_ID=PH02-BE-ACC-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
