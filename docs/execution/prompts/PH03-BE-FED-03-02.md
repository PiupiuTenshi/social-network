# PH03-BE-FED-03-02 — Nghiệm thu FED-03 và bàn giao

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-FED-03.
- Mục tiêu duy nhất: Nghiệm thu FED-03 và bàn giao.
- Miền: feed | Chức năng: FED-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-FED-03-02`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-FED-03-02
...
1. Rà soát FED-03 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Out-of-order; avatar update.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí FED-03 có report: Out-of-order; avatar update; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH03-BE-FED-03-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
