# PH05-FE-SET-04-P1-01 — Triển khai SET-04 P1: Xóa tài khoản

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: FE | Ưu tiên: P1 | Task cha: PH05-FE-SET-04-P1.
- Mục tiêu duy nhất: Triển khai SET-04 P1: Xóa tài khoản.
- Miền: cross-cutting | Chức năng: ACC-11.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH05-FE-SET-04-P1-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /thiet-lap/xoa-tai-khoan. Vai trò: Người dùng. Chỉ bật ACC-11 tại P1. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-FE-SET-04-P1-01
...
1. Mục tiêu screen: Giải thích ảnh hưởng, yêu cầu xác thực lại, tạo thời gian ân hạn và hỗ trợ hủy yêu cầu.
...
- **AC01**: Màn hình SET-04 tại /thiet-lap/xoa-tai-khoan thực hiện đúng quyền Người dùng và capability ACC-11.
- **AC02**: Trạng thái riêng được kiểm chứng: - Chưa yêu cầu; - Xác thực lại; - Đang trong thời gian ân hạn; - Đang đối soát xóa; - Đã hoàn tất; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH05-FE-SET-04-P1-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
