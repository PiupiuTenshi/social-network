# PH06-DATA-AI-P2-01 — Thiết kế lược đồ AI P2

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: DATA | Ưu tiên: P2 | Task cha: PH06-DATA-AI-P2.
- Mục tiêu duy nhất: Thiết kế lược đồ AI P2.
- Miền: ai | Chức năng: nền tảng/quy trình.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-DATA-AI-P2-01`; chưa được tạo chỉ vì có trong prompt.
- ai_embedding/chunk/job/transcript/interest vector; model version, ACL, pgvector, checkpoint/rebuild

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-DATA-AI-P2-01
...
1. Cụ thể hóa ai_embedding/chunk/job/transcript/interest vector; model version, ACL, pgvector, checkpoint/rebuild.
2. Đối chiếu ERD nguồn và hợp đồng đã khóa; chọn index từ query, mapping UTC/version, data classification.

## Tiêu chí chấp nhận

- **AC01**: ERD và mapping cụ thể cho ai_embedding/chunk/job/transcript/interest vector; model version, ACL, pgvector, checkpoint/rebuild; không lấn ownership.
...
Ghi `TASK_ID=PH06-DATA-AI-P2-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
