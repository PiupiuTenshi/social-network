# PH04-FE-RTC-03-P1-01 — Triển khai RTC-03 P1: Chia sẻ màn hình

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: FE | Ưu tiên: P1 | Task cha: PH04-FE-RTC-03-P1.
- Mục tiêu duy nhất: Triển khai RTC-03 P1: Chia sẻ màn hình.
- Miền: cross-cutting | Chức năng: RTC-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-FE-RTC-03-P1-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /cong-dong/:id/phong/:roomId/chia-se. Vai trò: Thành viên có quyền. Chỉ bật RTC-03 tại P1. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-FE-RTC-03-P1-01
...
1. Mục tiêu screen: Chọn nguồn màn hình, xem trước và theo dõi trạng thái phát trong phòng.
...
- **AC01**: Màn hình RTC-03 tại /cong-dong/:id/phong/:roomId/chia-se thực hiện đúng quyền Thành viên có quyền và capability RTC-03.
- **AC02**: Trạng thái riêng được kiểm chứng: - Chưa chia sẻ; - Đang chọn nguồn; - Đang phát; - Mạng yếu; - Không có quyền; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH04-FE-RTC-03-P1-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
