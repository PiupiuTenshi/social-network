# PH05-BE-DELETION-07 — Xóa tài khoản: Kiểm thử tích hợp hủy/purge/race/duplicate, ẩn <=5 phút, grace 30 ngày, PII <=45 ngày

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-DELETION.
- Mục tiêu duy nhất: Xóa tài khoản: Kiểm thử tích hợp hủy/purge/race/duplicate, ẩn <=5 phút, grace 30 ngày, PII <=45 ngày.
...
- Nhánh đề xuất: `feat/PH05-BE-DELETION-07`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-DELETION-07
...
1. Owner xử lý: cross-cutting; Kiểm thử tích hợp hủy/purge/race/duplicate, ẩn <=5 phút, grace 30 ngày, PII <=45 ngày.
2. Chỉ qua contract/event; ghi ack cùng transaction, lặp an toàn và có metric.

## Tiêu chí chấp nhận

- **AC01**: Kiểm thử tích hợp hủy/purge/race/duplicate, ẩn <=5 phút, grace 30 ngày, PII <=45 ngày có bằng chứng test và artifact được đối soát.
...
Ghi `TASK_ID=PH05-BE-DELETION-07`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
