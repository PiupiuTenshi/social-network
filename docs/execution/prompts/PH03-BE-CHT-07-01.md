# PH03-BE-CHT-07-01 — Triển khai CHT-07: Xác nhận đã đọc

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-CHT-07.
- Mục tiêu duy nhất: Triển khai CHT-07: Xác nhận đã đọc.
- Miền: chat | Chức năng: CHT-07.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-CHT-07-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Trò chuyện. Hợp đồng tham chiếu: POST read / MarkRead. Dữ liệu: conversation_member. Sự kiện: ReadUpdated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-CHT-07-01
...
1. Theo hợp đồng đã khóa: Chỉ tăng con trỏ last_read và phát xác nhận đã đọc.
2. Thực thi: Cursor chỉ tăng; membership.
3. Khảo sát src/Services/Chat, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của CHT-07, when thực hiện hợp lệ, then Chỉ tăng con trỏ last_read và phát xác nhận đã đọc.
- **AC02**: Kiểm soát được chứng minh: Cursor chỉ tăng; membership.
- **AC03**: Ca tối thiểu chạy thật: Không lùi; message sai.
...
Ghi `TASK_ID=PH03-BE-CHT-07-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
