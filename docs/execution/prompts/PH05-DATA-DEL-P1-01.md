# PH05-DATA-DEL-P1-01 — Thiết kế lược đồ DEL P1

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: DATA | Ưu tiên: P1 | Task cha: PH05-DATA-DEL-P1.
- Mục tiêu duy nhất: Thiết kế lược đồ DEL P1.
...
- Nhánh đề xuất: `feat/PH05-DATA-DEL-P1-01`; chưa được tạo chỉ vì có trong prompt.
- account_deletion_request/ack theo dịch vụ, grace/purge/cancel, retention_hold; không tạo DB chung

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-DATA-DEL-P1-01
...
1. Cụ thể hóa account_deletion_request/ack theo dịch vụ, grace/purge/cancel, retention_hold; không tạo DB chung.
2. Đối chiếu ERD nguồn và hợp đồng đã khóa; chọn index từ query, mapping UTC/version, data classification.

## Tiêu chí chấp nhận

- **AC01**: ERD và mapping cụ thể cho account_deletion_request/ack theo dịch vụ, grace/purge/cancel, retention_hold; không tạo DB chung; không lấn ownership.
...
Ghi `TASK_ID=PH05-DATA-DEL-P1-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
