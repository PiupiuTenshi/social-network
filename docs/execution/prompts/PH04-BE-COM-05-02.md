# PH04-BE-COM-05-02 — Nghiệm thu COM-05 và bàn giao

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-COM-05.
- Mục tiêu duy nhất: Nghiệm thu COM-05 và bàn giao.
- Miền: community | Chức năng: COM-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-COM-05-02`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-COM-05-02
...
1. Rà soát COM-05 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Mute/kick/ban; target cao hơn.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí COM-05 có report: Mute/kick/ban; target cao hơn; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH04-BE-COM-05-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
