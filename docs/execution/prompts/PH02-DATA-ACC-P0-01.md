# PH02-DATA-ACC-P0-01 — Thiết kế lược đồ ACC P0

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: DATA | Ưu tiên: P0 | Task cha: PH02-DATA-ACC-P0.
- Mục tiêu duy nhất: Thiết kế lược đồ ACC P0.
...
- Nhánh đề xuất: `feat/PH02-DATA-ACC-P0-01`; chưa được tạo chỉ vì có trong prompt.
- account, profile, refresh_token, account_setting; token hash/family/version và email/username unique

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH02-DATA-ACC-P0-01
...
1. Cụ thể hóa account, profile, refresh_token, account_setting; token hash/family/version và email/username unique.
2. Đối chiếu ERD nguồn và hợp đồng đã khóa; chọn index từ query, mapping UTC/version, data classification.

## Tiêu chí chấp nhận

- **AC01**: ERD và mapping cụ thể cho account, profile, refresh_token, account_setting; token hash/family/version và email/username unique; không lấn ownership.
...
Ghi `TASK_ID=PH02-DATA-ACC-P0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
