# PH05-BE-DELETION-03 — Xóa tài khoản: Ẩn PII profile, giữ lịch sử người khác theo policy, xử lý quyền/reconnect

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-DELETION.
- Mục tiêu duy nhất: Xóa tài khoản: Ẩn PII profile, giữ lịch sử người khác theo policy, xử lý quyền/reconnect.
...
- Nhánh đề xuất: `feat/PH05-BE-DELETION-03`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-DELETION-03
...
1. Owner xử lý: chat; Ẩn PII profile, giữ lịch sử người khác theo policy, xử lý quyền/reconnect.
2. Chỉ qua contract/event; ghi ack cùng transaction, lặp an toàn và có metric.

## Tiêu chí chấp nhận

- **AC01**: Ẩn PII profile, giữ lịch sử người khác theo policy, xử lý quyền/reconnect có bằng chứng test và artifact được đối soát.
...
Ghi `TASK_ID=PH05-BE-DELETION-03`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
