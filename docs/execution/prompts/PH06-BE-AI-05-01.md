# PH06-BE-AI-05-01 — Triển khai AI-05: Chuyển lời nói thành văn bản

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-AI-05.
- Mục tiêu duy nhất: Triển khai AI-05: Chuyển lời nói thành văn bản.
- Miền: ai | Chức năng: AI-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-AI-05-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: AI và tìm kiếm. Hợp đồng tham chiếu: POST transcriptions/worker. Dữ liệu: ai_job, transcript. Sự kiện: TranscriptReady.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-AI-05-01
...
1. Theo hợp đồng đã khóa: Tiến trình nền tải đối tượng đã cấp quyền -> xử lý bằng whisper.cpp -> lưu bản chép lời -> phát sự kiện cho Trò chuyện.
2. Thực thi: Authorized object; duration; concurrency.
3. Khảo sát src/Services/AiSearch, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của AI-05, when thực hiện hợp lệ, then Tiến trình nền tải đối tượng đã cấp quyền -> xử lý bằng whisper.cpp -> lưu bản chép lời -> phát sự kiện cho Trò chuyện.
- **AC02**: Kiểm soát được chứng minh: Authorized object; duration; concurrency.
- **AC03**: Ca tối thiểu chạy thật: Fail giữ audio; retry/DLQ.
...
Ghi `TASK_ID=PH06-BE-AI-05-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
