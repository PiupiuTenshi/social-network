# PH00-DATA-BOUNDARY-01 — Khóa ownership, ERD P0 và chiến lược dữ liệu

## Mục tiêu và phạm vi

- Phase: PH00 | Nhóm: DATA | Ưu tiên: P0 | Task cha: PH00-DATA-BOUNDARY.
- Mục tiêu duy nhất: Khóa ownership, ERD P0 và chiến lược dữ liệu.
...
- Nhánh đề xuất: `docs/PH00-DATA-BOUNDARY-01`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH00-DATA-BOUNDARY-01
...
1. Đối chiếu ERD trong DOCX/PDF với DATA_MODEL; cụ thể hóa bảng Chat/Feed còn mô tả tóm tắt.
2. Khóa tài khoản DB riêng, giao dịch/outbox/inbox, seed test, version/unique, retention và migration runner.
3. Ghi chính sách rebuild khi sự kiện đã quá retention; không dùng Kafka như bản sao lưu DB.

## Tiêu chí chấp nhận

- **AC01**: Mỗi bảng/aggregate có đúng một owner; sơ đồ không có FK/query xuyên DB.
- **AC02**: Schema P0 được chấp thuận, chỉ mục gắn access pattern; projection có export/checkpoint/rebuild.
- **AC03**: Kế hoạch migration từ rỗng/nâng cấp và backup/restore có lệnh cần xác minh, owner và tiêu chí RPO/RTO.
...
Ghi `TASK_ID=PH00-DATA-BOUNDARY-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
