# PH06-BE-AI-03-01 — Triển khai AI-03: Trợ lý RAG

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-AI-03.
- Mục tiêu duy nhất: Triển khai AI-03: Trợ lý RAG.
- Miền: ai | Chức năng: AI-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-AI-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: AI và tìm kiếm. Hợp đồng tham chiếu: POST /ai/ask. Dữ liệu: chunks, request metadata. Sự kiện: AiToken/AiCompleted.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-AI-03-01
...
1. Theo hợp đồng đã khóa: Truy xuất các chunk đã cấp quyền -> tạo prompt -> gọi Ollama -> truyền phản hồi tăng dần qua SignalR hoặc luồng HTTP.
2. Thực thi: Scoped retrieval; prompt limit; rate limit.
3. Khảo sát src/Services/AiSearch, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của AI-03, when thực hiện hợp lệ, then Truy xuất các chunk đã cấp quyền -> tạo prompt -> gọi Ollama -> truyền phản hồi tăng dần qua SignalR hoặc luồng HTTP.
- **AC02**: Kiểm soát được chứng minh: Scoped retrieval; prompt limit; rate limit.
- **AC03**: Ca tối thiểu chạy thật: Prompt injection; AI down.
...
Ghi `TASK_ID=PH06-BE-AI-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
