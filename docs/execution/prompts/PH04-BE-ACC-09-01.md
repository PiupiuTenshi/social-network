# PH04-BE-ACC-09-01 — Triển khai ACC-09: Đặt lại mật khẩu bằng OTP

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-ACC-09.
- Mục tiêu duy nhất: Triển khai ACC-09: Đặt lại mật khẩu bằng OTP.
- Miền: account | Chức năng: ACC-09.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-ACC-09-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: POST /auth/reset-password. Dữ liệu: account, reset token, refresh token. Sự kiện: UserPasswordReset.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-ACC-09-01
...
1. Theo hợp đồng đã khóa: Xác minh otp_hash, thời hạn và số lần thử -> băm mật khẩu mới -> cập nhật account -> đánh dấu token đã dùng -> thu hồi mọi refresh token.
2. Thực thi: OTP một lần; số lần thử; password policy.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-09, when thực hiện hợp lệ, then Xác minh otp_hash, thời hạn và số lần thử -> băm mật khẩu mới -> cập nhật account -> đánh dấu token đã dùng -> thu hồi mọi refresh token.
- **AC02**: Kiểm soát được chứng minh: OTP một lần; số lần thử; password policy.
- **AC03**: Ca tối thiểu chạy thật: OTP sai/hết hạn; thu hồi mọi phiên.
...
Ghi `TASK_ID=PH04-BE-ACC-09-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
