# PH04-BE-MED-04-01 — Triển khai MED-04: Xử lý phương tiện

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-MED-04.
- Mục tiêu duy nhất: Triển khai MED-04: Xử lý phương tiện.
- Miền: media | Chức năng: MED-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-MED-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tiến trình nền Phương tiện. Hợp đồng tham chiếu: Worker. Dữ liệu: media_asset. Sự kiện: MediaReady/Failed.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-MED-04-01
...
1. Theo hợp đồng đã khóa: Kiểm tra chữ ký tệp -> tùy chọn đổi kích thước ảnh và loại siêu dữ liệu -> chuyển sang Ready hoặc Failed.
2. Thực thi: Magic bytes; bounded concurrency.
3. Khảo sát src/Services/Media, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MED-04, when thực hiện hợp lệ, then Kiểm tra chữ ký tệp -> tùy chọn đổi kích thước ảnh và loại siêu dữ liệu -> chuyển sang Ready hoặc Failed.
- **AC02**: Kiểm soát được chứng minh: Magic bytes; bounded concurrency.
- **AC03**: Ca tối thiểu chạy thật: Poison media; retry/DLQ.
...
Ghi `TASK_ID=PH04-BE-MED-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
