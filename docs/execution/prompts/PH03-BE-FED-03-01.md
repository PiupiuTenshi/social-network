# PH03-BE-FED-03-01 — Triển khai FED-03: Cập nhật bản chiếu và làm mất hiệu lực bộ nhớ đệm

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-FED-03.
- Mục tiêu duy nhất: Triển khai FED-03: Cập nhật bản chiếu và làm mất hiệu lực bộ nhớ đệm.
- Miền: feed | Chức năng: FED-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-FED-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Bảng tin. Hợp đồng tham chiếu: Kafka consumer. Dữ liệu: summaries, cache. Sự kiện: Nhận update/delete.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-FED-03-01
...
1. Theo hợp đồng đã khóa: Cập nhật bản tóm tắt cục bộ rồi làm mất hiệu lực các khóa bộ nhớ đệm liên quan.
2. Thực thi: Event version; invalidation.
3. Khảo sát src/Services/Feed, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của FED-03, when thực hiện hợp lệ, then Cập nhật bản tóm tắt cục bộ rồi làm mất hiệu lực các khóa bộ nhớ đệm liên quan.
- **AC02**: Kiểm soát được chứng minh: Event version; invalidation.
- **AC03**: Ca tối thiểu chạy thật: Out-of-order; avatar update.
...
Ghi `TASK_ID=PH03-BE-FED-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
