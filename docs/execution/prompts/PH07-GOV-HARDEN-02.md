# PH07-GOV-HARDEN-02 — Rà soát bảo mật và chuỗi cung ứng

## Mục tiêu và phạm vi

- Phase: PH07 | Nhóm: GOV | Ưu tiên: P0 | Task cha: PH07-GOV-HARDEN.
- Mục tiêu duy nhất: Rà soát bảo mật và chuỗi cung ứng.
...
- Nhánh đề xuất: `feat/PH07-GOV-HARDEN-02`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH07-GOV-HARDEN-02
...
1. Chạy BOLA/XSS/SSRF/JWT/OTP/RAG injection, secret/dependency/container/license scan và SBOM.
2. Diễn tập rotation/least privilege; xử lý Critical/High trước đóng.

## Tiêu chí chấp nhận

- **AC01**: Không có secret thật/Critical/High tồn đọng; SBOM và scan gắn cùng candidate SHA.
- **AC02**: RAG private filter, nội bộ service JWT/Kafka ACL, upload và moderation đạt negative tests.
...
Ghi `TASK_ID=PH07-GOV-HARDEN-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
