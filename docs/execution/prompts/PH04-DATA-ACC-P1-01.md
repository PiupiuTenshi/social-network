# PH04-DATA-ACC-P1-01 — Thiết kế lược đồ ACC P1

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: DATA | Ưu tiên: P1 | Task cha: PH04-DATA-ACC-P1.
- Mục tiêu duy nhất: Thiết kế lược đồ ACC P1.
...
- Nhánh đề xuất: `feat/PH04-DATA-ACC-P1-01`; chưa được tạo chỉ vì có trong prompt.
- password_reset_token, account_oauth, avatar refs; OTP hash/attempt/TTL và unique provider identity

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-DATA-ACC-P1-01
...
1. Cụ thể hóa password_reset_token, account_oauth, avatar refs; OTP hash/attempt/TTL và unique provider identity.
2. Đối chiếu ERD nguồn và hợp đồng đã khóa; chọn index từ query, mapping UTC/version, data classification.

## Tiêu chí chấp nhận

- **AC01**: ERD và mapping cụ thể cho password_reset_token, account_oauth, avatar refs; OTP hash/attempt/TTL và unique provider identity; không lấn ownership.
...
Ghi `TASK_ID=PH04-DATA-ACC-P1-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
