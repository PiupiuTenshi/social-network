# PH04-BE-MED-01-01 — Triển khai MED-01: Tạo phiên tải lên

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-MED-01.
- Mục tiêu duy nhất: Triển khai MED-01: Tạo phiên tải lên.
- Miền: media | Chức năng: MED-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-MED-01-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Phương tiện. Hợp đồng tham chiếu: POST /media/uploads. Dữ liệu: upload_session. Sự kiện: Không.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-MED-01-01
...
1. Theo hợp đồng đã khóa: Kiểm tra siêu dữ liệu khai báo -> sinh khóa đối tượng -> tạo UploadSession -> sinh URL PUT ký trước sống ngắn.
2. Thực thi: MIME/size/purpose; key server sinh.
3. Khảo sát src/Services/Media, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MED-01, when thực hiện hợp lệ, then Kiểm tra siêu dữ liệu khai báo -> sinh khóa đối tượng -> tạo UploadSession -> sinh URL PUT ký trước sống ngắn.
- **AC02**: Kiểm soát được chứng minh: MIME/size/purpose; key server sinh.
- **AC03**: Ca tối thiểu chạy thật: Sai loại/kích thước; rate limit.
...
Ghi `TASK_ID=PH04-BE-MED-01-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
