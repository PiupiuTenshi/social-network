# PH02-BE-ACC-03-01 — Triển khai ACC-03: Xoay vòng refresh token

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH02-BE-ACC-03.
- Mục tiêu duy nhất: Triển khai ACC-03: Xoay vòng refresh token.
- Miền: account | Chức năng: ACC-03.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-BE-ACC-03-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Tài khoản. Hợp đồng tham chiếu: POST /auth/refresh. Dữ liệu: refresh_token family. Sự kiện: Audit tùy chọn.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH02-BE-ACC-03-01
...
1. Theo hợp đồng đã khóa: Xác minh hash -> thu hồi token cũ -> tạo token thay thế -> cấp access token và refresh token mới.
2. Thực thi: Xoay một lần; phát hiện tái sử dụng; giao dịch.
3. Khảo sát src/Services/Account, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của ACC-03, when thực hiện hợp lệ, then Xác minh hash -> thu hồi token cũ -> tạo token thay thế -> cấp access token và refresh token mới.
- **AC02**: Kiểm soát được chứng minh: Xoay một lần; phát hiện tái sử dụng; giao dịch.
- **AC03**: Ca tối thiểu chạy thật: Token cũ, token hết hạn, race refresh.
...
Ghi `TASK_ID=PH02-BE-ACC-03-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
