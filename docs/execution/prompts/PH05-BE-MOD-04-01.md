# PH05-BE-MOD-04-01 — Triển khai MOD-04: Đóng báo cáo và kiểm toán

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-MOD-04.
- Mục tiêu duy nhất: Triển khai MOD-04: Đóng báo cáo và kiểm toán.
- Miền: social | Chức năng: MOD-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH05-BE-MOD-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội - khối điều hành. Hợp đồng tham chiếu: POST /reports/{id}/close. Dữ liệu: content_report. Sự kiện: Completed/Rejected.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-MOD-04-01
...
1. Theo hợp đồng đã khóa: Ghi kết luận -> chuyển Closed/Rejected -> đặt thời hạn evidence -> tạo thông báo phù hợp cho người báo cáo mà không lộ thông tin nội bộ.
2. Thực thi: If-Match; audit; evidence retention.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của MOD-04, when thực hiện hợp lệ, then Ghi kết luận -> chuyển Closed/Rejected -> đặt thời hạn evidence -> tạo thông báo phù hợp cho người báo cáo mà không lộ thông tin nội bộ.
- **AC02**: Kiểm soát được chứng minh: If-Match; audit; evidence retention.
- **AC03**: Ca tối thiểu chạy thật: Close stale; reopen policy.
...
Ghi `TASK_ID=PH05-BE-MOD-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
