# PH03-BE-SOC-01-01 — Triển khai SOC-01: Theo dõi và bỏ theo dõi

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-SOC-01.
- Mục tiêu duy nhất: Triển khai SOC-01: Theo dõi và bỏ theo dõi.
- Miền: social | Chức năng: SOC-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-SOC-01-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội. Hợp đồng tham chiếu: POST/DELETE /users/{id}/follow. Dữ liệu: follow. Sự kiện: UserFollowed/Unfollowed.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-SOC-01-01
...
1. Theo hợp đồng đã khóa: Theo dõi: thêm quan hệ duy nhất; bỏ theo dõi: xóa quan hệ theo cách xử lý lặp an toàn.
2. Thực thi: Cấm self; chặn hai chiều; unique.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của SOC-01, when thực hiện hợp lệ, then Theo dõi: thêm quan hệ duy nhất; bỏ theo dõi: xóa quan hệ theo cách xử lý lặp an toàn.
- **AC02**: Kiểm soát được chứng minh: Cấm self; chặn hai chiều; unique.
- **AC03**: Ca tối thiểu chạy thật: Theo dõi lặp; blocked.
...
Ghi `TASK_ID=PH03-BE-SOC-01-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
