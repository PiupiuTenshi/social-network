...
1. Rà soát MKT-02 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Search down -> DB fallback.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí MKT-02 có report: Search down -> DB fallback; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH06-BE-MKT-02-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
