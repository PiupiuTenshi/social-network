# PH04-BE-COM-01-01 — Triển khai COM-01: Tạo và cập nhật cộng đồng

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-COM-01.
- Mục tiêu duy nhất: Triển khai COM-01: Tạo và cập nhật cộng đồng.
- Miền: community | Chức năng: COM-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-COM-01-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Cộng đồng. Hợp đồng tham chiếu: POST/PATCH /communities. Dữ liệu: community, member, role, channel. Sự kiện: CommunityCreated/Updated.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-COM-01-01
...
1. Theo hợp đồng đã khóa: Tạo membership chủ sở hữu, vai trò và kênh mặc định trong một giao dịch; cập nhật Cộng đồng bằng version.
2. Thực thi: Owner; If-Match; transaction.
3. Khảo sát src/Services/Community, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của COM-01, when thực hiện hợp lệ, then Tạo membership chủ sở hữu, vai trò và kênh mặc định trong một giao dịch; cập nhật Cộng đồng bằng version.
- **AC02**: Kiểm soát được chứng minh: Owner; If-Match; transaction.
- **AC03**: Ca tối thiểu chạy thật: Tạo mặc định; slug trùng; stale.
...
Ghi `TASK_ID=PH04-BE-COM-01-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
