# PH04-BE-CHT-09-01 — Triển khai CHT-09: Lời mời cuộc gọi

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH04-BE-CHT-09.
- Mục tiêu duy nhất: Triển khai CHT-09: Lời mời cuộc gọi.
- Miền: chat | Chức năng: CHT-09.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-BE-CHT-09-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Trò chuyện + cộng đồng. Hợp đồng tham chiếu: POST call-invites. Dữ liệu: call invite metadata. Sự kiện: CallInvite/CallStateChanged.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-BE-CHT-09-01
...
1. Theo hợp đồng đã khóa: Tạo trạng thái lời mời nhẹ -> phát SignalR tới người nhận -> khi chấp nhận thì chuyển sang luồng cấp token LiveKit.
2. Thực thi: Membership/block/privacy; timeout.
3. Khảo sát src/Services/Chat, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của CHT-09, when thực hiện hợp lệ, then Tạo trạng thái lời mời nhẹ -> phát SignalR tới người nhận -> khi chấp nhận thì chuyển sang luồng cấp token LiveKit.
- **AC02**: Kiểm soát được chứng minh: Membership/block/privacy; timeout.
- **AC03**: Ca tối thiểu chạy thật: Reject/timeout không rò resource.
...
Ghi `TASK_ID=PH04-BE-CHT-09-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
