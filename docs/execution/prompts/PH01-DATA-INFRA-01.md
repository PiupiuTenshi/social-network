# PH01-DATA-INFRA-01 — Hạ tầng dữ liệu core và migration runner

## Mục tiêu và phạm vi

- Phase: PH01 | Nhóm: DATA | Ưu tiên: P0 | Task cha: PH01-DATA-INFRA.
- Mục tiêu duy nhất: Hạ tầng dữ liệu core và migration runner.
...
- Nhánh đề xuất: `feat/PH01-DATA-INFRA-01`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH01-DATA-INFRA-01
...
1. Docker Compose PostgreSQL/Kafka/Valkey; SeaweedFS cấu hình hạ tầng, chưa mở tính năng P1.
2. Tách DB/credentials/topic prefix dev/test/staging, healthcheck/volume/resource limit.
3. Runner migration độc quyền theo DB; kiểm restart container không mất dữ liệu đã commit.

## Tiêu chí chấp nhận

- **AC01**: Một lệnh dựng hạ tầng, healthcheck và volumes đạt; thiếu cấu hình fail fast.
- **AC02**: Credential một dịch vụ không truy cập DB dịch vụ khác; không public cổng DB/broker.
- **AC03**: Chỉ một runner migration; không auto-migrate từ mỗi replica; có test restart/restore tối thiểu.
...
Ghi `TASK_ID=PH01-DATA-INFRA-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
