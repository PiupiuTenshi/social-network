# PH03-BE-CHT-02-01 — Triển khai CHT-02: Gửi tin nhắn

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-CHT-02.
- Mục tiêu duy nhất: Triển khai CHT-02: Gửi tin nhắn.
- Miền: chat | Chức năng: CHT-02.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-CHT-02-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Trò chuyện. Hợp đồng tham chiếu: SignalR/POST messages. Dữ liệu: message, outbox. Sự kiện: MessageSent + MessageCreated. P0 chỉ nội dung văn bản; từ chối media refs chưa hỗ trợ theo quyết định P0, mở ở P1.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-CHT-02-01
...
1. Theo hợp đồng đã khóa: Lưu message và Outbox trong một giao dịch -> phát SignalR tới nhóm đang trực tuyến -> phát sự kiện Kafka bất đồng bộ.
2. Thực thi: Membership; clientMessageId; rate limit.
3. Khảo sát src/Services/Chat, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của CHT-02, when thực hiện hợp lệ, then Lưu message và Outbox trong một giao dịch -> phát SignalR tới nhóm đang trực tuyến -> phát sự kiện Kafka bất đồng bộ.
- **AC02**: Kiểm soát được chứng minh: Membership; clientMessageId; rate limit.
- **AC03**: Ca tối thiểu chạy thật: Gửi lặp; DB fail không ghost.
...
Ghi `TASK_ID=PH03-BE-CHT-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
