...
1. Theo hợp đồng đã khóa: Kiểm tra bộ nhớ đệm Valkey -> đọc FeedDb khi không có -> áp dụng con trỏ và giới hạn -> trả DTO đầy đủ.
2. Thực thi: Cursor ổn định; hard filter.
3. Khảo sát src/Services/Feed, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của FED-02, when thực hiện hợp lệ, then Kiểm tra bộ nhớ đệm Valkey -> đọc FeedDb khi không có -> áp dụng con trỏ và giới hạn -> trả DTO đầy đủ.
- **AC02**: Kiểm soát được chứng minh: Cursor ổn định; hard filter.
- **AC03**: Ca tối thiểu chạy thật: Cache miss; không lộ private.
...
Ghi `TASK_ID=PH03-BE-FED-02-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
