# PH02-BE-ACC-01-02 — Nghiệm thu ACC-01 và bàn giao

## Mục tiêu và phạm vi

- Phase: PH02 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH02-BE-ACC-01.
- Mục tiêu duy nhất: Nghiệm thu ACC-01 và bàn giao.
- Miền: account | Chức năng: ACC-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH02-BE-ACC-01-02`; chưa được tạo chỉ vì có trong prompt.
...
```powershell
python -X utf8 scripts/task_gate.py check PH02-BE-ACC-01-02
...
1. Rà soát ACC-01 với dữ liệu/HTTP/sự kiện thật; mở rộng từ happy path sang các ca Đăng ký thành công, trùng, dữ liệu sai, Outbox.
2. Chạy unit/integration/contract/architecture phù hợp, format/analyzer/build; đồng bộ tài liệu và handoff.

## Tiêu chí chấp nhận

- **AC01**: Tất cả tiêu chí ACC-01 có report: Đăng ký thành công, trùng, dữ liệu sai, Outbox; không test bỏ qua hoặc lỗi Critical/High.
...
Ghi `TASK_ID=PH02-BE-ACC-01-02`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
