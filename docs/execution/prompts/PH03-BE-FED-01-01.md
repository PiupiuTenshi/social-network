# PH03-BE-FED-01-01 — Triển khai FED-01: Xây dựng bản chiếu bảng tin theo dõi

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-FED-01.
- Mục tiêu duy nhất: Triển khai FED-01: Xây dựng bản chiếu bảng tin theo dõi.
- Miền: feed | Chức năng: FED-01.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-FED-01-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Bảng tin. Hợp đồng tham chiếu: Kafka consumer. Dữ liệu: feed_entry, summaries, inbox. Sự kiện: Nhận Post/Follow/Profile.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-FED-01-01
...
1. Theo hợp đồng đã khóa: Xử lý sự kiện lặp an toàn -> tạo hoặc cập nhật FeedEntry, FeedPostSummary và FeedUserSummary khi cần.
2. Thực thi: Idempotency; privacy/block.
3. Khảo sát src/Services/Feed, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của FED-01, when thực hiện hợp lệ, then Xử lý sự kiện lặp an toàn -> tạo hoặc cập nhật FeedEntry, FeedPostSummary và FeedUserSummary khi cần.
- **AC02**: Kiểm soát được chứng minh: Idempotency; privacy/block.
- **AC03**: Ca tối thiểu chạy thật: Duplicate; rebuild; lag.
...
Ghi `TASK_ID=PH03-BE-FED-01-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
