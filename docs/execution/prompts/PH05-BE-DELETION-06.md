# PH05-BE-DELETION-06 — Xóa tài khoản: Đối soát ack/retry/timeouts, cancel trong grace, chặn completed khi thiếu participant

## Mục tiêu và phạm vi

- Phase: PH05 | Nhóm: BE | Ưu tiên: P1 | Task cha: PH05-BE-DELETION.
- Mục tiêu duy nhất: Xóa tài khoản: Đối soát ack/retry/timeouts, cancel trong grace, chặn completed khi thiếu participant.
...
- Nhánh đề xuất: `feat/PH05-BE-DELETION-06`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH05-BE-DELETION-06
...
1. Owner xử lý: account; Đối soát ack/retry/timeouts, cancel trong grace, chặn completed khi thiếu participant.
2. Chỉ qua contract/event; ghi ack cùng transaction, lặp an toàn và có metric.

## Tiêu chí chấp nhận

- **AC01**: Đối soát ack/retry/timeouts, cancel trong grace, chặn completed khi thiếu participant có bằng chứng test và artifact được đối soát.
- **AC02**: Không query/ghi DB dịch vụ khác; thiếu ack không được completed; contract participant đã khóa.
...
Ghi `TASK_ID=PH05-BE-DELETION-06`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
