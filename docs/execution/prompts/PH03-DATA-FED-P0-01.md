# PH03-DATA-FED-P0-01 — Thiết kế lược đồ FED P0

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: DATA | Ưu tiên: P0 | Task cha: PH03-DATA-FED-P0.
- Mục tiêu duy nhất: Thiết kế lược đồ FED P0.
- Miền: feed | Chức năng: nền tảng/quy trình.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-DATA-FED-P0-01`; chưa được tạo chỉ vì có trong prompt.
- FeedEntry/UserSummary/PostSummary, Inbox/checkpoint; quyền/block, invalidation và rebuild

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-DATA-FED-P0-01
...
1. Cụ thể hóa FeedEntry/UserSummary/PostSummary, Inbox/checkpoint; quyền/block, invalidation và rebuild.
2. Đối chiếu ERD nguồn và hợp đồng đã khóa; chọn index từ query, mapping UTC/version, data classification.

## Tiêu chí chấp nhận

- **AC01**: ERD và mapping cụ thể cho FeedEntry/UserSummary/PostSummary, Inbox/checkpoint; quyền/block, invalidation và rebuild; không lấn ownership.
...
Ghi `TASK_ID=PH03-DATA-FED-P0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
