# PH04-FE-AUTH-04-P1-01 — Triển khai AUTH-04 P1: Quên mật khẩu

## Mục tiêu và phạm vi

- Phase: PH04 | Nhóm: FE | Ưu tiên: P1 | Task cha: PH04-FE-AUTH-04-P1.
- Mục tiêu duy nhất: Triển khai AUTH-04 P1: Quên mật khẩu.
- Miền: cross-cutting | Chức năng: ACC-08.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH04-FE-AUTH-04-P1-01`; chưa được tạo chỉ vì có trong prompt.
- Route: /quen-mat-khau. Vai trò: Khách. Chỉ bật ACC-08 tại P1. Các chức năng ưu tiên cao hơn và gap chưa khóa phải tắt.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH04-FE-AUTH-04-P1-01
...
1. Mục tiêu screen: Yêu cầu mã OTP mà không tiết lộ email có tồn tại trong hệ thống hay không.
...
- **AC01**: Màn hình AUTH-04 tại /quen-mat-khau thực hiện đúng quyền Khách và capability ACC-08.
- **AC02**: Trạng thái riêng được kiểm chứng: - Rỗng; - Đang gửi; - Phản hồi chung; - Giới hạn tần suất; - SMTP chậm nhưng yêu cầu đã được ghi nhận; ; Ngoài các trạng thái trên, áp dụng trạng thái dùng chung trong `sources/11-state-matrix.md`.
...
Ghi `TASK_ID=PH04-FE-AUTH-04-P1-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
