# G0-01 — Nghiệm thu kiến trúc trước khi viết nghiệp vụ

## Mục tiêu và phạm vi

- Phase: PH00 | Nhóm: GATE | Ưu tiên: P0 | Task cha: G0.
- Mục tiêu duy nhất: Nghiệm thu kiến trúc trước khi viết nghiệp vụ.
...
- Nhánh đề xuất: `docs/G0-01`; chưa được tạo chỉ vì có trong prompt.
- Bản kế hoạch không chứng minh mã ứng dụng đã tồn tại.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check G0-01
...
1. Rà soát bằng chứng của AUDIT, CONTRACT, BOUNDARY, TOOLCHAIN; ghi kết luận từng tiêu chí G0.

## Tiêu chí chấp nhận

- **AC01**: Ownership, schema P0, API/event/error đã khóa và không còn blocker trong phạm vi.
- **AC02**: ADR, sơ đồ, migration rỗng, kiểm thử kiến trúc có artifact thật được rà soát.
...
Ghi `TASK_ID=G0-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
