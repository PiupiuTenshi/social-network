# PH06-BE-FED-04-01 — Triển khai FED-04: Sinh ứng viên và chấm điểm gợi ý

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-FED-04.
- Mục tiêu duy nhất: Triển khai FED-04: Sinh ứng viên và chấm điểm gợi ý.
- Miền: feed | Chức năng: FED-04.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-FED-04-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Bảng tin và dữ liệu AI. Hợp đồng tham chiếu: GET /feed nội bộ. Dữ liệu: recommendation cache. Sự kiện: Không.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-FED-04-01
...
1. Theo hợp đồng đã khóa: Kết hợp ứng viên từ theo dõi, phổ biến, ngữ nghĩa và cộng tác -> chấm điểm -> điều chỉnh đa dạng -> lấy N kết quả đầu.
2. Thực thi: Privacy trước trả; diversity.
3. Khảo sát src/Services/Feed, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của FED-04, when thực hiện hợp lệ, then Kết hợp ứng viên từ theo dõi, phổ biến, ngữ nghĩa và cộng tác -> chấm điểm -> điều chỉnh đa dạng -> lấy N kết quả đầu.
- **AC02**: Kiểm soát được chứng minh: Privacy trước trả; diversity.
- **AC03**: Ca tối thiểu chạy thật: Cold start; AI down fallback.
...
Ghi `TASK_ID=PH06-BE-FED-04-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
