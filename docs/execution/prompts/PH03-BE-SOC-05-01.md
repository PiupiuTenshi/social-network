# PH03-BE-SOC-05-01 — Triển khai SOC-05: Xóa bài viết

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-SOC-05.
- Mục tiêu duy nhất: Triển khai SOC-05: Xóa bài viết.
- Miền: social | Chức năng: SOC-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-SOC-05-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Mạng xã hội. Hợp đồng tham chiếu: DELETE /posts/{id}. Dữ liệu: post. Sự kiện: PostDeleted.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-SOC-05-01
...
1. Theo hợp đồng đã khóa: Xóa mềm bài viết và ghi Outbox PostDeleted; bộ nhận sự kiện xóa hoặc ẩn các bản chiếu liên quan.
2. Thực thi: Owner/moderator; soft delete; lặp an toàn.
3. Khảo sát src/Services/Social, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của SOC-05, when thực hiện hợp lệ, then Xóa mềm bài viết và ghi Outbox PostDeleted; bộ nhận sự kiện xóa hoặc ẩn các bản chiếu liên quan.
- **AC02**: Kiểm soát được chứng minh: Owner/moderator; soft delete; lặp an toàn.
- **AC03**: Ca tối thiểu chạy thật: Xóa lặp; projection được dọn.
...
Ghi `TASK_ID=PH03-BE-SOC-05-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
