# PH06-BE-FED-05-01 — Triển khai FED-05: Cập nhật embedding sở thích

## Mục tiêu và phạm vi

- Phase: PH06 | Nhóm: BE | Ưu tiên: P2 | Task cha: PH06-BE-FED-05.
- Mục tiêu duy nhất: Triển khai FED-05: Cập nhật embedding sở thích.
- Miền: ai | Chức năng: FED-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH06-BE-FED-05-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: AI và tìm kiếm. Hợp đồng tham chiếu: Background worker. Dữ liệu: interest embedding. Sự kiện: EmbeddingGenerated tùy chọn.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH06-BE-FED-05-01
...
1. Theo hợp đồng đã khóa: Xử lý theo lô hoặc bất đồng bộ để suy ra vector sở thích từ embedding của nội dung đã tương tác.
2. Thực thi: Loại dữ liệu không còn quyền; decay.
3. Khảo sát src/Services/AiSearch, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của FED-05, when thực hiện hợp lệ, then Xử lý theo lô hoặc bất đồng bộ để suy ra vector sở thích từ embedding của nội dung đã tương tác.
- **AC02**: Kiểm soát được chứng minh: Loại dữ liệu không còn quyền; decay.
- **AC03**: Ca tối thiểu chạy thật: Backpressure; retry.
...
Ghi `TASK_ID=PH06-BE-FED-05-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
