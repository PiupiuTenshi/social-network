# PH03-BE-CHT-05-01 — Triển khai CHT-05: Trạng thái đang nhập

## Mục tiêu và phạm vi

- Phase: PH03 | Nhóm: BE | Ưu tiên: P0 | Task cha: PH03-BE-CHT-05.
- Mục tiêu duy nhất: Triển khai CHT-05: Trạng thái đang nhập.
- Miền: chat | Chức năng: CHT-05.
- Chế độ: implementation. Chỉ sửa đầu ra trong phạm vi subtask; không làm task kế tiếp.
- Nhánh đề xuất: `feat/PH03-BE-CHT-05-01`; chưa được tạo chỉ vì có trong prompt.
- Owner: Trò chuyện/SignalR. Hợp đồng tham chiếu: SignalR Typing. Dữ liệu: Valkey TTL tùy chọn. Sự kiện: TypingChanged.

## Cổng bắt buộc trước khi thực hiện

```powershell
python -X utf8 scripts/task_gate.py check PH03-BE-CHT-05-01
...
1. Theo hợp đồng đã khóa: Phát trạng thái đang nhập tạm thời; trạng thái tự dừng hoặc hết hạn.
2. Thực thi: Membership; throttle; ephemeral.
3. Khảo sát src/Services/Chat, tests và hợp đồng thực tế; bổ sung log/metric/trace, cancellation, test ngay cùng mã.

## Tiêu chí chấp nhận

- **AC01**: Given điều kiện trước của CHT-05, when thực hiện hợp lệ, then Phát trạng thái đang nhập tạm thời; trạng thái tự dừng hoặc hết hạn.
- **AC02**: Kiểm soát được chứng minh: Membership; throttle; ephemeral.
- **AC03**: Ca tối thiểu chạy thật: Drop khi offline; auto-expire.
...
Ghi `TASK_ID=PH03-BE-CHT-05-01`, mục tiêu, file/ký hiệu đã đổi, contract/ADR, từng AC → artifact → kết quả, lệnh thật, rủi ro và blocker.
